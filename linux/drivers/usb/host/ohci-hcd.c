...
static struct timer_list   ohciPollingModeDetect;
static struct usb_hcd *save_hcd = NULL;
#define POLLING_MODE_DETECT_INTV  1

void ohci_poll(unsigned long data)
{		
	if(save_hcd != NULL){ 
	...
	mod_timer(&ohciPollingModeDetect, jiffies + POLLING_MODE_DETECT_INTV);
	return;
}
#endif
...
#if defined(CONFIG_MIPS_TC3162U) && defined(TC_SUPPORT_3G)
	/*USB host change to polling mode. shnwind 20101129.*/
	init_timer(&ohciPollingModeDetect);
	ohciPollingModeDetect.expires = jiffies + POLLING_MODE_DETECT_INTV;
	ohciPollingModeDetect.function = ohci_poll;
	ohciPollingModeDetect.data = 0;
	add_timer(&ohciPollingModeDetect);
#endif
	return retval;
...
static void __exit ohci_hcd_mod_exit(void)
{
	...
	/*USB host change to polling mode. shnwind 20101129.*/
    //delete timer
	del_timer_sync(&ohciPollingModeDetect);
#endif	
#ifdef PCI_DRIVER
