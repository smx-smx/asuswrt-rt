...
struct timer_list   ehciPollingModeDetect;
static struct usb_hcd   *save_hcd = NULL;
...
{
	if(save_hcd != NULL){
		if(save_hcd->state == HC_STATE_RUNNING){
			ehci_irq (save_hcd);
		}
	}
	mod_timer(&ehciPollingModeDetect, jiffies + POLLING_MODE_DETECT_INTV);
	return;
}	
#endif
...
// this is deeply broken on almost all architectures
	...
	/*USB host change to polling mode, do not need interrupt here.
	        shnwind 20101129.*/
	ehci_writel(ehci, INTR_MASK,
		    &ehci->regs->intr_enable); /* Turn On Interrupts */
#endif
...
#if defined(CONFIG_MIPS_TC3162U) && defined(TC_SUPPORT_3G)
	/*USB host change to polling mode. shnwind 20101129.*/
	init_timer(&ehciPollingModeDetect);
	ehciPollingModeDetect.expires = jiffies + POLLING_MODE_DETECT_INTV;
	ehciPollingModeDetect.function = ehci_poll;
	ehciPollingModeDetect.data = 0;
	add_timer(&ehciPollingModeDetect);
#endif	
	return retval;
}
...
static void __exit ehci_hcd_cleanup(void)
{
	...
	/*USB host change to polling mode. shnwind 20101129.*/
	//delete timer
	del_timer_sync(&ehciPollingModeDetect);
#endif	
