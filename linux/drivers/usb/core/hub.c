...
static int get_hub_descriptor(struct usb_device *hdev, void *data, int size)
{
	...
	//add retry time for TC. shnwind 20100408.
	for (i = 0; i < TC_RETRY_TIMES; i++) {
#else
	for (i = 0; i < 3; i++) {
#endif	
...
#if defined(CONFIG_MIPS_TC3162U)
		//go to fail if time out occured.
		if(ret == -ETIMEDOUT && i >= 3){
			return -EINVAL;
		}
#endif	
...
static int clear_port_feature(struct usb_device *hdev, int port1, int feature)
{
	...
	for (i = 0; i < TC_RETRY_TIMES && status < 0; i++) {
...
#if defined(CONFIG_MIPS_TC3162U)
		//break if time out occured.		
		if(status == -ETIMEDOUT && i>= USB_STS_RETRIES)
			break;	
#endif
	}
	return status;
	...
	for (i = 0; i < PORT_RESET_TRIES; i++) {
		...
		case 0:
			...
			if(hub_port_status(hub, port1, &portstatus, &portchange) == 0){	
				//printk("ret %x, portstatus %x\n",ret,portstatus);
				if(!(portstatus & USB_PORT_STAT_ENABLE)){
					set_port_feature(hub->hdev,
						port1, USB_HOST_LIGHT_RESET);
...
#if defined(CONFIG_MIPS_TC3162U)
			//go to fail if time out occured.
			if (retval >= 0 || (retval == -ETIMEDOUT && j >= SET_ADDRESS_TRIES))
#else
			if (retval >= 0)
#endif			
				break;
