...
__DMEM static struct list_head ptype_all __read_mostly;	/* Taps */
...
__IMEM void __netif_rx_schedule(struct net_device *dev)
{
	unsigned long flags;
...
gso:
#if defined(CONFIG_CPU_TC3162) || defined(CONFIG_MIPS_TC3262)
	{
		...
		rc = dev_gso_hard_start_xmit(skb, dev);
		if (rc != -9999)
			return rc;
	}
#else
...
gso:
	...
	if (q->enqueue && !(skb->vlan_tag_flag&(1<<30))) {
#else
        if (q->enqueue) {
#endif
		/* Grab device queue */
...
__DMEM DEFINE_PER_CPU(struct netif_rx_stats, netdev_rx_stat) = { 0, };
 ...
 */
#ifdef CONFIG_TC_VOIP
unsigned int mod_skb_off = 22;
extern unsigned short int* kernel_voip_rx_port;
typedef int (*send_to_cm_t)(char* buf, int byteCnt, int channel);
extern send_to_cm_t send_to_cm ;
EXPORT_SYMBOL(mod_skb_off);
...
#ifdef CONFIG_TC_VOIP
	int i = 0;
	unsigned int tmp_skb_port,tmp_skb_ppp_LCP;
...
#ifdef CONFIG_TC_VOIP
	/*Voip enqueue depend*/
	if (queue->voip_queue.qlen <= netdev_max_backlog) {
		if (queue->voip_queue.qlen) {
voip_enqueue:
			dev_hold(skb->dev);
			//unsigned int tmp_skb_port = *((unsigned short int*)(skb + 22+mod_skb_off));
			tmp_skb_port = *((unsigned short int*)(skb->data + 22));
			if(dbgEnable)
				printk("tmp skb port %d\n",tmp_skb_port);
			if(kernel_voip_rx_port){
        			for(i=0;i<4;i++){
                		if((kernel_voip_rx_port[i]!=0) && (tmp_skb_port == kernel_voip_rx_port[i])){
                			/*if(dbgEnable)
                				printk(">>>>VoIPRTP en q \n");*/
                			__skb_queue_tail(&queue->voip_queue, skb);
                			local_irq_restore(flags);
                			return NET_RX_SUCCESS;
                		}
                	}
        	}
			/*20100115 pork added for VoiP drop mechanism*/
			/*there are several kinds of packets going to be inserted into VoIP queue.
			 * the reason why inserting these kinds of packets is preventing they were droped by ip_rcv.
			 * the following is the list of packets into VoIP queue:
			 * 1. SIP packet (shift:22, dst port:5060)
			 * 2. DHCP packet (shift:22, dst port:68)
			 * 3. PPP discover (shift:1 CODE field)
			 * 4. PPP configuration (shift:6 LCP field)
			 * 5. PPP ECHO REPLY (shift:6 LCP field)
			 * 6. loopback interface packets (dev name)*/
			tmp_skb_ppp_LCP = *(unsigned short int*)(skb->data+6);
			tmp_skb_ppp_CODE = *(unsigned char*)(skb->data+1);
			if((tmp_skb_port == 5060) || (strncmp(skb->dev->name, "lo", 2)==0) || (tmp_skb_port ==68)||\
					(tmp_skb_ppp_LCP == 0xc021) || (tmp_skb_ppp_LCP == 0xc223) ||\
					(tmp_skb_ppp_LCP == 0x8021) || (tmp_skb_ppp_LCP == 0x80fd) ||\
					(tmp_skb_ppp_CODE == 0x07)||(tmp_skb_ppp_CODE == 0x65)){
				if(dbgEnable){
					printk(">>>>VoIP en q port:%d dev name:%s PPP LCP:0x%02x PPP CODE: 0x%x\n",tmp_skb_port,skb->dev->name,tmp_skb_ppp_LCP,tmp_skb_ppp_CODE);
				}
				__skb_queue_tail(&queue->voip_queue, skb);
				local_irq_restore(flags);
				return NET_RX_SUCCESS;
...
struct net_bridge_fdb_entry *(*br_fdb_get_hook)(struct net_bridge *br,
						unsigned char *addr);
__DMEM void (*br_fdb_put_hook)(struct net_bridge_fdb_entry *ent) __read_mostly;

/*
 * If bridge module is loaded call bridging hook.
 *  returns NULL if packet was consumed.
 */
__DMEM struct sk_buff *(*br_handle_frame_hook)(struct net_bridge_port *p,
...
__IMEM static int process_backlog(struct net_device *backlog_dev, int *budget)
{
	int work = 0;
	...
	for (;;) {
		...
		local_irq_disable();
#ifdef CONFIG_TC_VOIP
		skb = __skb_dequeue(&queue->voip_queue);
		if(!skb){
			skb = __skb_dequeue(&queue->input_pkt_queue);
			if (!skb){
				goto job_done;
			}
			else{// pork modified at 20100108
				work++;
			}
		}
		else{
		...
		if(downstream_qos_enable){
		for( i = 0 ; i < PRIORITY_QUEUE_NUM; i++){
			skb = __skb_dequeue(&queue->pri_queue[i]);
			if(skb != NULL){
				break;
			}
			}
			...
			//need dequeue priority queue. 
			skb = NULL;	
		}	
		if(skb == NULL){
			//no packet in priority queue.
#endif			
			skb = __skb_dequeue(&queue->input_pkt_queue);
		if (!skb)
			goto job_done;
...
#ifndef CONFIG_TC_VOIP
		work++;
#endif

#ifdef CONFIG_TC_VOIP
		if ((voip_work + work) >= quota || (jiffies - start_time > 1)){
			if(dbgEnable)
				printk("voip_work:%d work:%d quota:%d \n",voip_work,work,quota);
#else
