...
//use 'uname -r' to show current version : 2.6.22.15
...
int do_match_test(unsigned char match_type,  const char* reference, char* query)
{
	...
	switch(match_type)
	{
		...
		case WEBURL_CONTAINS_NOCASE_TYPE:
			matches = (stristr(query, reference) != NULL);//Ren: it is case insensitive.
			break;
...
static struct ipt_match weburl_match  __read_mostly  = 
{
	.name		= "weburl",
	.match		= &match,
	.family		= AF_INET,
