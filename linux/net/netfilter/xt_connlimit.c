...
static inline bool is_udp_icmp_connection(const struct nf_conn *conn)
{
	u_int16_t proto = conn->tuplehash[0].tuple.dst.protonum;
		      ...
		      const struct xt_match *match)
{
	...
	list_for_each_entry_safe(conn, tmp, hash, list) {
		...
		if (found_ct != NULL &&
		    ...
		    if(!is_udp_icmp_connection(found_ct) && !already_closed(found_ct)){
				/*
			...
			if(is_udp_icmp_connection(found_ct))
				addit = false;
		}
...
static int connlimit_match(const struct sk_buff *skb,
...
static int connlimit_check(const char *tablename, const void *ip,
