			    ...
			    const char *dptr)
{
	...
	if ( ct->layer7.app_proto ) {
		...
		ct->layer7.app_proto = NULL; /* don't free again */
	}
	
	ct->layer7.app_proto = 
			kmalloc(strlen("rtp")+1, GFP_ATOMIC);
	...
	if(!ct->layer7.app_proto){
		...
		strcpy(ct->layer7.app_proto, "rtp");
	}
		    ...
		    enum ip_conntrack_info ctinfo)
{
	...
	if (ct_sip_get_info(ct, dptr, datalen, &matchoff, &matchlen, pos) > 0) {
		...
		if (ct_sip_get_info(ct, dptr, datalen, &matchoff, &matchlen,
			...
			/*printk("<<<<<<<SIP packet>>>>>>>>\n%s \n\n==============\nSDP: addr:0x%08lx port: 0x%08lx\n============\n",dptr,addr,htons(port));*/
