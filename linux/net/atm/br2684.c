...
static int get_MAC_from_flash(unsigned char *mac_addr)
{
	...
	int i;
	for (i=0; i<6; i++) {
#ifdef TCSUPPORT_NAND_FLASH	//for 128KB/block
		flash_mac_addr[i] = READ_FLASH_BYTE(flash_base + 0x1ff48 + i);
#else
...
__IMEM static void br2684_push(struct atm_vcc *atmvcc, struct sk_buff *skb)
{
	...
	if (likely((brvcc->encaps == e_llc))) {
		/* let us waste some time for checking the encapsulation.
		   Note, that only 7 char is checked so frames with a valid FCS
		   are also accepted (but FCS is not checked of course) */
		if ((skb->len >= sizeof(llc_oui_pid_pad)) &&
		    (memcmp(skb->data, llc_oui_pid_pad, 7) == 0) &&
            (brdev->payload == p_bridged)) //add this line to fix Bug#8296 --Trey 
        {
			if (skb->data[7] == 0x01)
...
static int br2684_mac_addr_routed(struct net_device *dev, void *p){
	//printk("ERROR : br2684 routed set macaddr\n");
	return 0;
}
...
static void br2684_setup_routed(struct net_device *netdev)
{
	...
	/*Br2684 routed mode do not need set mac addr.
		There is no mac header here. shnwind modify 20100920*/
	/*my_eth_mac_addr = netdev->set_mac_address;
	netdev->set_mac_address = br2684_mac_addr;*/
	netdev->set_mac_address = br2684_mac_addr_routed;
	netdev->hard_start_xmit = br2684_start_xmit;
	netdev->get_stats = br2684_get_stats;
	netdev->addr_len = 0;
	netdev->mtu = 1500;
