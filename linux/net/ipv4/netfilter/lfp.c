...
static inline int lfp_func(int pf, unsigned int hook, struct sk_buff *skb)
{
	...
	if(skb->vlan_tag_flag & NFC_LFP_ENABLE) return 1;
	...
	for (i = 0, p = &lfp_ports[0]; i < NR_ALL_LFP_PORTS; ++i, ++p) {
		...
		if (port == p->port) {
			...
			skb->vlan_tag_flag |= NFC_LFP_ENABLE;
			ret = 1;
			break;
		}
	}
...
static int __init init(void)
{
	...
	p = create_proc_entry("lfpctrl", 0600, proc_net);
	...
	p = create_proc_entry("lfp_ports_ctrl", 0600, proc_net);
	if (p) {
...
#ifdef CONFIG_PROC_FS
	remove_proc_entry("lfp_ports_ctrl", proc_net);
	remove_proc_entry("lfpctrl", proc_net);
#endif
}
