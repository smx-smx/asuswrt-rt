 ...
 *
 * Version:	$Id: //BBN_Linux/Branch/Branch_for_SDK_Release_MultiChip_20111013/tclinux_phoenix/linux/net/ipv4/arp.c#1 $
 *
				   ...
				   struct neighbour *n)
{
	...
	seq_printf(seq, "%-16s 0x%-10x0x%-10x%s     *        %s %x\n",
		   tbuf, hatype, arp_state_to_flags(n), hbuffer, dev->name, n->nud_state);
	read_unlock(&n->lock);
}
