 ...
 *
 * Version:	$Id: //BBN_Linux/Branch/Branch_for_SDK_Release_MultiChip_20111013/tclinux_phoenix/linux/net/ipv4/tcp.c#1 $
 *
 * Authors:	Ross Biro
...
#ifdef CONFIG_IP_NF_LFP
	if (lfp_ip) {
		if (inet->saddr == lfp_ip)
			port = ntohs(inet->sport);
		else if (inet->daddr == lfp_ip)
			port = ntohs(inet->dport);
		...
		int optname, char __user *optval, int optlen)
{
	...
	case TCP_LFP:
		...
		if (lfp_ip == inet->saddr)
			port = ntohs(inet->sport);
		else if (lfp_ip == inet->daddr)
			port = ntohs(inet->dport);
