...
#ifdef CONFIG_TC3162_IMEM		
		if(!strcmp(secstrings + sechdrs[i].sh_name, ".text.imem")){
			unsigned char mod;

			if((mod = next_dest & 3) != 0)
				next_dest += (4 - mod);
			if(next_dest + sechdrs[i].sh_size > (unsigned int)(&__imem) + CONFIG_IMEM_SIZE){
				printk("The remaining IMEM space cannot accommodate section %s !!\r\n", secstrings + sechdrs[i].sh_name);
				printk("Remaining IMEM space: %d bytes\tSection Size: %d bytes\r\n", imem_remains, sechdrs[i].sh_size);
			}
			else{
				dest = next_dest;
				next_dest += sechdrs[i].sh_size;
				imem_remains = (unsigned int)(&__imem) + CONFIG_IMEM_SIZE - next_dest;
				refill = 1;
			}
		}
