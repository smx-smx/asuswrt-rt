...
do
	...
	ENET0=`/userfs/bin/tcapi get $node_name eth0.1`
	ENET1=`/userfs/bin/tcapi get $node_name eth0.2`
	if [ "$is2PSupported" != "Yes" ] 
	then
		ENET2=`/userfs/bin/tcapi get $node_name eth0.3`
		ENET3=`/userfs/bin/tcapi get $node_name eth0.4`
	fi
...
then 
	...
	echo "<tr><td align=\"center\" class=\"tabdata\">$j</td><td align=\"center\" class=\"tabdata\">$ACTIVATE</td><td align=\"center\" class=\"tabdata\"> $ENET0$ENET1$USB0$RA0$RA1$RA2$RA3</td><td align=\"center\" class=\"tabdata\"><div>$DESMAC</div><div>$DESIP/$DESMASK</div><div>$DESPORTRANGEBEGIN~$DESPORTRANGEEND</div></td><td align=\"center\" class=\"tabdata\"><div>$SRCMAC</div><div>$SRCIP/$SRCMASK</div><div>$SRCPORTRANGEBEGIN~$SRCPORTRANGEEND</div></td><td align=\"center\" class=\"tabdata\">$PROTOCOLID</td><td align=\"center\" class=\"tabdata\"><div>$VLANIDRANGEBEGIN</div><div>|</div><div>$VLANIDRANGEEND</div></td>"
fi
