...
var swap_ram_usage_array = new Array(31);
...
function render_SwapRAM(total, free, used){
	var swap_pt = "";
	var swap_used_percentage = 0;
	var swap_total_MB = 0, swap_free_MB = 0, swap_used_MB =0;
	
	swap_total_MB = Math.round(total/1024);
	swap_free_MB = Math.round(free/1024);
	swap_used_MB = Math.round(used/1024);
	
	$('swapram_total_info').innerHTML = swap_total_MB + "MB";
	$('swapram_free_info').innerHTML = swap_free_MB + "MB";
	$('swapram_used_info').innerHTML = swap_used_MB + "MB";
	
	swap_used_percentage = Math.round((used/total)*100);
	$('swapram_bar').style.width = swap_used_percentage +"%";
	$('swapram_bar').style.width = swap_used_percentage +"%";
	$('swapram_quantification').innerHTML = swap_used_percentage +"%";
	
	swap_ram_usage_array.push(100 - swap_used_percentage);
	swap_ram_usage_array.splice(0,1);
	
	for(i=0;i<31;i++){
		swap_pt += i*9 +","+ swap_ram_usage_array[i] + " ";
	}

	$('swapram_graph').setAttribute('points', swap_pt);
}
...
function detect_RAM_Status(){
	$j.ajax({
    	...
    	success: function(){
				...
				if("<% tcWebApi_get("WebCustom_Entry", "isSwapFileSupport", "s" ) %>" == "Yes" && SwapTotal != "0"){
						document.getElementById("SwapRAM_graph_table").style.display = "";
						render_SwapRAM(SwapTotal, SwapFree, SwapUsed);					
				}else{
						document.getElementById("SwapRAM_graph_table").style.display = "none";
				}	
...
<input type="hidden" name="current_page" value="/cgi-bin/device-map/router_status.asp">
...
<tr>
	<td> 
		...
		<table id="SwapRAM_graph_table" width="98%" border="1" align="center" cellpadding="4" cellspacing="0" class="table1px">	
			<tr>
				<td colspan="3">		
					<div class="title">Virtual RAM</div>
					<img class="ling_image" src="/images/New_ui/networkmap/linetwo2.png">
				</td>
			</tr>
			<tr class="ram_table">
				<td>
					<div>Used</div>	  			
					<div id="swapram_used_info"></div>	
				</td>
				<td>
					<div>Free</div>
					<div id="swapram_free_info"></div>	  			
				</td>
				<td>
					<div>Total</div>	  			
					<div id="swapram_total_info"></div>	  			
				</td>
			</tr>  
			<tr>
				<td colspan="3">
					<div>
						<table>
							<tr>
								<td class="loading_bar" colspan="2">
									<div>
										<div id="swapram_bar" class="status_bar"></div>				
									</div>
								</td>
								<td class="percentage_bar">
									<div id="swapram_quantification">0%</div>
								</td>
							</tr>
						</table>
					</div>
				...
				<td colspan="3">
					<div style="margin:0px 11px 0px 11px;background-color:black;">
						<svg width="270px" height="100px">
							...
							<polyline id="swapram_graph" style="fill:none;stroke:#FF6900;stroke-width:1;width:200px;" points=""></polyline>
						</svg>
					</div>
				</td>
			</tr>
