...
var cloud_sync = decodeURIComponentSafe('<% tcWebApi_Get("AiCloud_Entry","cloud_sync","s") %>');
...
function showInvitation(){
	...
	cal_panel_block("invitation", 0.25);
	if(!isInvite){
		document.getElementById("invitationInfo").innerHTML = "<br/> <% tcWebApi_Get("SysInfo_Entry","aicloud_invitation_invalid","s") %>";
		...
		htmlCode += "<tr height='40px'><td width='30%'><% tcWebApi_Get("String_Entry","IPC_autofwDesc_in","s") %></td><td style='font-weight:bolder;'>" + decode_array[0] + "</td></tr>";
		...
		htmlCode += "<tr height='40px'><td width='30%'><% tcWebApi_Get("String_Entry","sync_router_localfolder","s") %></td><td>";
		htmlCode += "<input type='text' id='PATH_rs' class='input_20_table' style='margin-left:0px;height:23px;' name='cloud_dir' value='' onclick=''/>";
		htmlCode += "<input name='button' type='button' class='button_gen_short' style='margin-left:5px;' onclick='get_disk_tree(1);document.getElementById(\"folderTree_panel\").style.marginLeft+=30;' value='<% tcWebApi_Get("String_Entry","Cloudsync_browser_folder","s") %>'/>";
		htmlCode += "</td></tr>";
		...
		if(decode_array[3] != ""){
			...
			htmlCode += "<span style='color:#FC0;display:none;margin-left:5px;' id='codeHint'><% tcWebApi_Get("String_Entry","JS_Invalid_Vcode","s") %></span></td></tr>";
		}
    	...
    	success: function(response){
					...
					if(document.getElementById("cloudStatus_dropbox")){
						...
						if(cloud_dropbox_obj != ""){
							...
							_cloud_dropbox_msg += "<% tcWebApi_Get("String_Entry","aicloud_no_record","s") %>";
						}
					...
					if(document.getElementById("cloudStatus_ftpclient")){
						...
						if(cloud_ftpclient_obj != ""){
							...
							_cloud_ftpclient_msg += "<% tcWebApi_Get("String_Entry","aicloud_no_record","s") %>";
						}
					...
					if(document.getElementById("cloudStatus_usbclient")){
						...
						if(cloud_usbclient_obj != ""){
							...
							_cloud_usbclient_msg += "<% tcWebApi_Get("String_Entry","aicloud_no_record","s") %>";
						}
					...
					if(document.getElementById("cloudStatus_sambaclient")){
						...
						if(cloud_sambaclient_obj != ""){
							...
							_cloud_sambaclient_msg += "<% tcWebApi_Get("String_Entry","aicloud_no_record","s") %>";
						}
...
function validform(){	
	...
	/*if(document.form.cloud_password.value.length < 8){ //disable to check length of password temporary, Jieming added at 2013.08.13
		alert("<% tcWebApi_Get("String_Entry","cloud_list_password","s") %>");
		return false;
	}*/
	...
	$j("#povider_tr").hover(
		function(){     // for mouse enter event
			...
			if(isSupport(ss_support, "usb"))
				$j('#Usb').parent().css('display','block');
...
function dropbox_login(){
	...
	var callback_url = window.location.href.slice(0,b) + "/cgi-bin/dropbox_callback.asp,onDropBoxLogin"; 
...
<div id="invitationBg" class="invitepopup_bg">
	<div id="invitation" class="panel_folder" style="margin-top:30px;">
		<table>
			<tr>
				<td>
					<div class="machineName" style="font-family:Microsoft JhengHei;font-size:12pt;font-weight:bolder; margin-top:20px;margin-left:30px;"><% tcWebApi_Get("String_Entry","Cloudsync_Get_Invitation","s") %></div>	<!-- You have got a new invitation! -->
				</td>
			</tr>
		</table>
...
<div id="folderTree_panel" class="panel_folder" style="z-index:1000;">
	<table><tr><td>
		...
		<td>
			<div style="width:240px;margin-top:14px;margin-left:135px;">
				<table >
					<tr>
						<td><div id="createFolderBtn" class="createFolderBtn" title="<% tcWebApi_Get("String_Entry","AddFolderTitle","s") %>"></div></td>
						<td><div id="deleteFolderBtn" class="deleteFolderBtn" title="<% tcWebApi_Get("String_Entry","DelFolderTitle","s") %>"></div></td>
						<td><div id="modifyFolderBtn" class="modifyFolderBtn" title="<% tcWebApi_Get("String_Entry","ModFolderTitle","s") %>"></div></td>
					</tr>
				</table>
			</div>
		</td></tr></table>
	...
	<div style="background-image:url(/images/Tree/bg_02.png);background-repeat:no-repeat;height:90px;">		
		...
		<input class="button_gen" type="button" id="btn_confirm_folder0" onclick="confirm_folderTree0();" value="<% tcWebApi_Get("String_Entry","CTL_ok","s") %>" style="display:none;">
		<input class="button_gen" type="button" id="btn_confirm_folder" onclick="confirm_folderTree();" value="<% tcWebApi_Get("String_Entry","CTL_ok","s") %>" style="display:none;">
	</div>
</div>
					...
					<table width="97%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3" class="FormTable" id="cloudAddTable" style="margin-top:10px;margin-bottom:10px;">
								...
								<td>				
									...
									<ul id="povider_tr" class="navigation" style="margin:-15px 0 0 -39px;*margin:-20px 0 0 0;">  
										<li >
											<dl>
												<dt>
													<div id="select_service" style="height:35px;margin:-4px 0px 0px 0px;padding:4px;" value=""><% tcWebApi_Get("String_Entry","AddAccountTitle","s") %></div>
												</dt> 
						...
						<tr style="display:none;">
							<th width="30%" style="font-family: Calibri;font-weight: bolder;">
								<% tcWebApi_Get("String_Entry","Server_Name","s") %>	<!-- Server Name -->
							</th>			
							<td>
							  ...
							  &nbsp;
							  <span><% tcWebApi_Get("String_Entry","feedback_optional","s") %></span>
							</td>
						</tr>	
						<tr style="display:none;">
							<th width="30%" style="font-family: Calibri;font-weight: bolder;">
								<% tcWebApi_Get("String_Entry","WA11a_ExAuthDBIPAddr_in","s") %>
							</th>			
							<td>
								<span>smb://</span>
						...
						<tr style="display:none;">
							<th width="30%" style="font-family: Calibri;font-weight: bolder;">
								<% tcWebApi_Get("String_Entry","Cloudsync_Shared_Folder","s") %><!-- Server share folder -->
							</th>			
							<td>
						...
						<tr style="display:none;">
							...
							<td>
							  ...
							  <input name="button" type="button" class="button_gen_short" onclick="get_disk_tree(0);" value="<% tcWebApi_Get("String_Entry","Cloudsync_browser_folder","s") %>"/>
								<div id="noUSB0" style="color:#FC0;display:none;margin-left: 3px;"><% tcWebApi_Get("String_Entry","no_usb_found","s") %></div>
							</td>
						</tr>	
						...
						<tr style="display:none;">
							<th width="30%" style="font-family: Calibri;font-weight: bolder;">
								<% tcWebApi_Get("String_Entry","IPC_VSList_ftpport","s") %>
							</th>			
							<td>
						...
						<tr style="display:none;">
							<th width="30%" style="font-family: Calibri;font-weight: bolder;">
								<% tcWebApi_Get("String_Entry","Cloudsync_Remote_Path","s") %>	<!-- Remote Path -->
							</th>			
							<td>
						  ...
						  <tr>
							<th width="30%" style="font-family: Calibri;font-weight: bolder;">
								<% tcWebApi_Get("String_Entry","routerSync_folder","s") %>
							</th>
							<td>
							<input type="text" id="PATH" class="input_32_table" style="height: 23px;" name="cloud_dir" value="" onclick="" autocomplete="off" autocorrect="off" autocapitalize="off"/>
		  					<input name="button" type="button" class="button_gen_short" onclick="get_disk_tree(1);" value="<% tcWebApi_Get("String_Entry","Cloudsync_browser_folder","s") %>"/>
						  ...
						  <tr>
							<th width="30%" style="font-family: Calibri;font-weight: bolder;">
								<% tcWebApi_Get("String_Entry","Cloudsync_Rule","s") %>
							</th>
							<td>
						  ...
						  <tr id="security_code_tr">
							<th width="30%" style="font-family: Calibri;font-weight: bolder;">
								<% tcWebApi_Get("String_Entry","routerSync_Security_code","s") %>
							</th>
							<td>
								<div style="color:#FC0;"><input id="security_code_field" name="security_code_field" type="text" maxlength="6" class="input_32_table" style="height: 23px;width:100px;margin-right:10px;" autocorrect="off" autocapitalize="off"><% tcWebApi_Get("String_Entry","OTP_Auth","s") %><!--OTP Authentication--></div>
							</td>
						  </tr>
						  <tr height="45px;" id="captcha_tr" style="display:none;">
							<th width="30%" style="font-family: Calibri;font-weight: bolder;">
								<% tcWebApi_Get("String_Entry","Captcha","s") %>	<!-- Captcha -->
							</th>			
							<td style="height:85px;">
								...
								<div id="captcha_hint" style="color:#FC0;height:25px;margin-top:10px;"><% tcWebApi_Get("String_Entry","Captcha_note","s") %></div>	<!-- Please input the captcha -->
								<div>
								...
								<div style="color:#FC0;text-decoration:underline;height:35px;margin:-35px 0px 0px 380px;cursor:pointer" onclick="refresh_captcha();"><% tcWebApi_Get("String_Entry","CTL_refresh","s") %></div>   
							</td>
						  </tr>
						</table>
							<div class="apply_gen" style="margin-top:20px;margin-bottom:10px;display:none;background-color: #2B373B;" id="applyDiv">
	  					<input name="button" type="button" class="button_gen" onclick="showAddTable();" value="<% tcWebApi_Get("String_Entry","CTL_Cancel","s") %>"/>
	  					<input id="applyBtn" name="button" type="button" class="button_gen" onclick="applyRule()" value="<% tcWebApi_Get("String_Entry","CTL_apply","s") %>"/>
			  ...
			  <tr>
					<td align="left" valign="top">
					  <table width="100%" border="0" cellpadding="5" cellspacing="0" class="FormTitle" id="FormTitle">
   					...
   					<table width="99%" border="1" align="center" cellpadding="4" cellspacing="0" class="FormTable_table" id="cloudlistTable" style="margin-top:30px;">
    					...
    					<th width="25%"><% tcWebApi_Get("String_Entry","HSDPAC_Username_in","s") %></a></th>
      					...
      					<th width="30%"><% tcWebApi_Get("String_Entry","FolderName","s") %></th>
						...
						<div id="cloud_synclist_Block">
							<table width="99%" cellspacing="0" cellpadding="4" align="center" class="list_table" id="cloud_synclist_table">
								<tbody>
									<tr height="55px">
										<td style="color:#FFCC00;" colspan="6"><% tcWebApi_Get("String_Entry","QKS_detect_sanglass","s") %>...</td>	<!-- Detecting -->
									</tr>
								</tbody>
							</table>
						</div>
	  				...
	  				<div class="apply_gen" id="creatBtn" style="margin-top:30px;display:none;">
							<input name="applybutton" id="applybutton" type="button" class="button_gen_long" onclick="showAddTable(9);" value="<% tcWebApi_Get("String_Entry","AddAccountTitle","s") %>" style="word-wrap:break-word;word-break:normal;">
