...
W('<input type="button" value="<%tcWebApi_get("String_Entry","CTL_refresh","s")%>" onclick="' + (exec ? exec : 'refreshClick()') + '" id="refresh-button"></div>');
}
...
for (var i = 0; i < arguments.length; ++i){
	if(arguments[i][1] == "PTM" && "<% tcWebApi_get("WebCustom_Entry","havePtm","s"); %>" != "Yes")
		continue;
	else if(arguments[i][1] == "Wireless (5GHz)" && band5g_support == -1)
		continue ;
	else	
