...
if(res.indexOf('Sun') != -1|| res.indexOf('Mon') != -1 || res.indexOf('Tue') != -1 || res.indexOf('Wed') != -1
|| res.indexOf('Thu') != -1 || res.indexOf('Fri') != -1 || res.indexOf('Sat') != -1)
...
cell = $(_cell);
date = indexDate(i);
cell.asp(formatDate(date, colFormat));
setDayID(cell, date);
});
}
...
slotScroller.height(bodyHeight - allDayHeight + 5);
slotHeight = slotTableFirstInner.height() + 4; // +1 for border
if (dateChanged) {
resetScroll();
}
}
...
var top;
var k;
segmentContainer[0].innerHTML = daySegHTML(segs); // faster than .asp()
...
var segCnt = segs.length;
var element;
tempContainer[0].innerHTML = daySegHTML(segs); // faster than .asp()
...
"start: '"+ new Date(y, m, d+test_startday, test_starttime, 0) +"',"+
"end: '"+ new Date(y, m, d+test_endday, test_endtime, 0) +"',"+
"allDay: false" +
"}";
...
{
id: jData.length+169, // 24(hr)*7(week)
title: title,
start: start,
end: end,
allDay: allDay
...
MULTIFILTER_MACFILTER_DAYTIME_row_time = "";
