	...
	for(i=0; i<10; i++){
		if(i == now){
			...
			if(dialnumber == HaveSettingDialNumber[i]){
				alert("<%tcWebApi_get("String_Entry","VoIPSpeedJS0Text","s")%>");
				return 1;
			}
		}
	}
...
function NotAValidDialNumber(){
	...
	for(i=0;i<len;i++)
	{
		...
		{
			alert("<%tcWebApi_get("String_Entry","VoIPSpeedJS1Text","s")%>");
			return 1;
		}
	}
...
function dosave(){
	...
	if(f.VoIPUserId.value.length <= 0){
		alert("<%tcWebApi_get("String_Entry","VoIPSpeedJS2Text","s")%>");
		return;
	}else{
		for(i = 0; i < f.VoIPUserId.value.length; i++) {
			tmp = f.VoIPUserId.value.charCodeAt(i);
			if(checkAuthName(tmp) != 0) {
				alert("<%tcWebApi_get("String_Entry","VoIPSpeedJS3Text","s")%>");
				return;
			}
		}
	}
	...
	if(f.VoIPSpeedDialNumber.value.length <= 0){
		alert("<%tcWebApi_get("String_Entry","VoIPSpeedJS4Text","s")%>");
		return;
	}else{
	...
	if(f.VoIPDestination.options[index].value=="Direct Call"){
		if(f.VoIPIpAddressOrHostName.value.length <= 0){
			alert("<%tcWebApi_get("String_Entry","VoIPSpeedJS5Text","s")%>");
			return;
		}else{
		...
		if(f.VoIPPort.value.length <= 0){
			alert("<%tcWebApi_get("String_Entry","VoIPSpeedJS7Text","s")%>");
			return;
		}
...
<FORM METHOD="POST" ACTION="/cgi-bin/voip_speed_dial.asp" name="VOIP_SPEED_DIAL_form"><table width="760" border="0" align=center cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
	...
	<font color="#FFFFFF"><%tcWebApi_get("String_Entry","VoIPSpeedDialText","s")%></font></td>
	<td width="10" class="black">&nbsp;</td><td width="150"></td><td width="10"></td><td width="440"></td></tr>	
	<tr>
		<td class="light-orange">&nbsp;</td>
		<td class="light-orange"></td>
		<td class="tabdata" align=right><font color="#000000"><%tcWebApi_get("String_Entry","VoIPSpeedIndexText","s")%></font></td>
	...
	<tr>
		<td class="light-orange">&nbsp;</td>
		<td class="light-orange"></td>
		<td class="tabdata" align=right><font color="#000000"><%tcWebApi_get("String_Entry","VoIPSpeedDestinationText","s")%></font></td>
		<td class="tabdata" align=center>:</td>
		...
		<td class="tabdata">
			<SELECT NAME="VoIPDestination" SIZE="1" onchange="directcheck()">
				<OPTION value="Proxy" ><%tcWebApi_get("String_Entry","VoIPSpeedProxyText","s")%>
			<!--	<OPTION value="Local Line" >Local Line -->
				<OPTION value="Direct Call" ><%tcWebApi_get("String_Entry","VoIPSpeedDirectCallText","s")%>
			</SELECT>
		</td></tr>
	<tr>
		<td class="light-orange">&nbsp;</td><td class="light-orange"></td>
		<td class="tabdata" align=right><font color="#000000"><%tcWebApi_get("String_Entry","VoIPSpeedDialNumberText","s")%></font></td>
		<td class="tabdata" align=center>:</td>
		<td class="tabdata">
	...
	<tr>
		<td class="light-orange">&nbsp;</td><td class="light-orange"></td>
		<td class="tabdata" align=right><font color="#000000"><%tcWebApi_get("String_Entry","VoIPSpeedUserIdText","s")%></font></td>
		<td class="tabdata" align=center>:</td>
		<td class="tabdata">
	...
	<tr>
		<td class="light-orange">&nbsp;</td><td class="light-orange"></td>
		<td class="tabdata" align=right><font color="#000000"><%tcWebApi_get("String_Entry","VoIPSpeedIPNameText","s")%></font></td>
		<td class="tabdata" align=center>:</td>
		<td class="tabdata">
	...
	<tr>
		<td class="light-orange">&nbsp;</td><td class="light-orange"></td>
		<td class="tabdata" align=right><font color="#000000"><%tcWebApi_get("String_Entry","VoIPSpeedPortText","s")%></font></td>
		<td class="tabdata" align=center>:</td>
		<td class="tabdata">
