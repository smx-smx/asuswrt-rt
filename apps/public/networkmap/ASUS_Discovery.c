...
const char asus_disc_path[] = "/var/tmp/asusdev.lst";
...
{	
...
#else
	char buf[256];	//Andy Chiu, 2014/12/01
	//get netmask of LAN
	if(!tcapi_get("Lan_Entry0", "netmask", buf))
		lan_netmask = inet_addr(buf);
	else
	{
		myAsusDiscoveryDebugPrint("Failed to get netmask of LAN\n");
		return 0;
	}

	//get LAN ip. The gateway of LAN is the same as LAN ip.
	if(!tcapi_get("Lan_Entry0", "IP", buf))
		lan_gateway = inet_addr(buf);
	else
	{
		myAsusDiscoveryDebugPrint("Failed to get LAN IP\n");
		return 0;
	}	
#endif
	...
	//Andy Chiu,, 2015/03/25
	//nvram_set("asus_device_list", asus_device_list_buf);
	char tmp[12];
	sprintf(tmp, "%d", a_GetRouterCount);
	tcapi_set("AsusDevLst_Common", "size", tmp);
	write_to_file(asus_disc_path, asus_device_list_buf, strlen(asus_device_list_buf), "w");
