...
// Get current hostname from DHCP leases
	...
	char value[256], node[20];
	int count = 0, num = 0;

	//check dhcp is enable
	if(tcapi_get("Lan_Dhcp", "type", value) || atoi(value) != 1)
		return 0;

	//get the number of dhcp lease list
	tcapi_get("DhcpLease", "LeaseNum", value);
	num = atoi(value);
	NMP_DEBUG_M("LeaseNum:%d\n", value);
	int i;
	for(i = 0; i < 253 && count < num; ++i)
	{
		sprintf(node, "DhcpLease_Entry%d", i);
		//check ip
		if(!tcapi_get(node, "IP", value))
		{
			NMP_DEBUG_M("[%s, %s]\n", ipaddr, value);
			if(!strcmp(ipaddr, value))
			{
				NMP_DEBUG_M("try to get host name\n");
				//get host name
				if(!tcapi_get(node, "HostName", value))
				{
					NMP_DEBUG_M("Find hostname(%s)\n", value);
					if(strlen(value) > 0 && !strchr(value, '*') && !strchr(value, ':'))
					{
						strncpy(p_client_detail_info_tab->device_name[p_client_detail_info_tab->detail_info_num], value, 15);
						NMP_DEBUG_M("Find hostname(%s)\n", value);
					}
				}
				break;
	...
	// Get names from static lease list, overruling anything else
	tcapi_get("LanHost", "HostNum", value);
	num = atoi(value);
	count = 0;
	NMP_DEBUG_M("Find static lease list\n");
	for(i = 0; i < 254 && count < num; ++i)
	{
		sprintf(node, "LanHost _Entry%d", i);
		//check ip
		if(!tcapi_get(node, "IP", value))
		{
			NMP_DEBUG_M("[%s, %s]\n", ipaddr, value);
...
int write_to_file(const char *file, const char *data, const int len, const char *file_flag)
{
	FILE *fp;
	size_t ret;
	if(!file || !data || !len || !file_flag)
		return -1;

	NMP_DEBUG("[%s, %d]%s, %s, %d, %s\n", __FUNCTION__, __LINE__, file, data, len, file_flag);
	fp= fopen(file, file_flag);
	if(fp)
	{
