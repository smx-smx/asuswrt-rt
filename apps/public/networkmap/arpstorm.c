...
int main()
{
	...
	//Get Router's IP/Mac
	NMP_DEBUG("start arpstorm\n");
#if 1
	if(tcapi_get("Lan_Entry0", "IP", router_ipaddr)  < 0 || tcapi_get("Info_Ether", "mac", router_mac) < 0)
	{
		perror("get lan ip or mac error.\n");
		//NMP_DEBUG("get lan ip or mac error.\n");
		return -1;
	}
	NMP_DEBUG("lan ip is %s, mac is %s\n", router_ipaddr, router_mac);
#else
