...
#if 0
int32 matrixUserNewKey(sslKeys_t **keys,int32 (*readNewCert)(void))
{
	int32 reVal;
	if(readNewCert == NULL || keys == NULL)
		return -1;
	matrixSslFreeKeys(*keys);
	*keys = NULL;
	
	/*Change the cert file content*/
	reVal = readNewCert(&(*key));
	#if 0
	matrixSslReadKeys(&(*key),NULL,NULL,NULL,Certfile);
	#endif
