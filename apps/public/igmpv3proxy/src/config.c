...
int loadConfig(char *configFile) {
    ...
    while ( token != NULL ) {
        ...
        else if(strcmp("igmp", token)==0) {
            if(updateIgmpConfig() < 0)
                my_log(LOG_ERR, 0, "handle token '%s' failed", token);
            // Read next token...
            token = nextConfigToken();
            continue;
        } else {
...
int updateIgmpConfig() {
	...
	my_log(LOG_DEBUG, 0, "handle token: '%s'", token);

	if(strcmp("robustness-variable", token)==0) {
		token = nextConfigToken();
		if(token == NULL) return -1;
		value = (unsigned int)strtoul(token, NULL, 0);
		commonConfig.robustnessValue = value;
		commonConfig.startupQueryCount = value;
		commonConfig.lastMemberQueryCount = value;
	}
	else if(strcmp("query-interval", token)==0) {
		token = nextConfigToken();
		if(token == NULL) return -1;
		value = (unsigned int)strtoul(token, NULL, 0);
		commonConfig.queryInterval = value;
		commonConfig.startupQueryInterval = value / 4;
	}
	else if(strcmp("query-response-interval", token)==0) {
		token = nextConfigToken();
		if(token == NULL) return -1;
		commonConfig.queryResponseInterval = strtoul(token, NULL, 0);
	}
	else if(strcmp("last-member-query-interval", token)==0) {
		token = nextConfigToken();
		if(token == NULL) return -1;
		commonConfig.lastMemberQueryInterval = strtoul(token, NULL, 0);
	}
	else {
		my_log(LOG_WARNING, 0, "Unknown token '%s' in configfile", token);
		return -1;
