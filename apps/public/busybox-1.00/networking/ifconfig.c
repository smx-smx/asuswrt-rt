 ...
 *
 * $Id: //BBN_Linux/Branch/Branch_for_SDK_Release_MultiChip_20111013/tclinux_phoenix/apps/public/busybox-1.00/networking/ifconfig.c#1 $
 *
 */
...
#define WAN_IF	"nas"
#define LAN_IF	"br"
#define WAN_MAC_KEYWD	"WAN_MAC="
#define LAN_MAC_KEYWD	"LAN_MAC="
static int in_ether(char *bufp, struct sockaddr *sap);
static int write2mac_conf(char *interface,char *mac);
#endif
				...
				if (mask & A_CAST_HOST_COPY) {
					...
					if (mask & A_CAST_RESOLVE) {
						...
						if (in_ether(host, &sa)) {
					 	...
					 	write2mac_conf(ifr.ifr_name, host);
						p = (char *) &sa;
					}
#endif
...
#ifdef CONFIG_FEATURE_IPV6
				/*Enable ipv6 when ifconfig interface up*/
				if(ifr.ifr_flags & (IFF_UP | IFF_RUNNING)){
					sprintf(pathbuf,"/proc/sys/net/ipv6/conf/%s/disable_ipv6",ifr.ifr_name);		
					fp = fopen(pathbuf, "r");
					if(fp){				
						if(fgets(ifdisable, 2, fp) != NULL){	
							if(strcmp(ifdisable,"1") == 0){
								sprintf(cmdbuf, "/bin/echo 0 > %s", pathbuf);
								system(cmdbuf);		
							}
						}	
					}
...
static int write2mac_conf(char *interface,char *mac_addr)
{
	FILE *fp = NULL;
	char buf[MAX_SIZE];
	char mac_info[2][32];
	...
	if(fp==NULL)
		return -1;
	for(i=0; fgets(buf,MAX_SIZE,fp); i++){
		strcpy(mac_info[i],buf);
	}
	...
	if(strstr(interface,WAN_IF) != NULL){
		sprintf(tmp_mac,"%s%s\n", WAN_MAC_KEYWD, mac_addr);
	}else if(strstr(interface, LAN_IF) != NULL){
		sprintf(tmp_mac,"%s%s\n", LAN_MAC_KEYWD, mac_addr);
	}
	
	for(i=0; i < 2; i++){
		if(strncmp(tmp_mac, mac_info[i],strlen(WAN_MAC_KEYWD)) == 0){
			strcpy(mac_info[i],tmp_mac);
		}
		fputs(mac_info[i],fp);
	}
	fclose(fp);
