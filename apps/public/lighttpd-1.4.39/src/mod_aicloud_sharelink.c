...
INIT_FUNC(mod_aicloud_sharelink_init) {
	plugin_data *p;
...
FREE_FUNC(mod_aicloud_sharelink_free) {
	plugin_data *p = p_d;
...
#if 0
SETDEFAULTS_FUNC(mod_aicloud_sharelink_set_defaults) {
	plugin_data *p = p_d;
	size_t i = 0;
...
SETDEFAULTS_FUNC(mod_aicloud_sharelink_set_defaults) {
	...
	for (i = 0; i < srv->config_context->used; i++) {
		...
		Cdbg(DBE, "usbdiskname=%s, s->alias->used=%d", usbdiskname, s->alias->used);
		#if 1
		if (s->alias->used >= 1) {
			...
			for (j = 0; j < a->used; j ++) {
				...
				for (k = j + 1; k < a->used; k ++) {
					...
					Cdbg(DBE, "key=%s, usbdiskname=%s", key->ptr, usbdiskname);
...
static int mod_aicloud_sharelink_patch_connection(server *srv, connection *con, plugin_data *p) {
	...
	PATCH(alias);
	//Cdbg(1, "mod_aicloud_sharelink_patch_connection..........");
#if EMBEDDED_EANBLE
	char *usbdiskname = nvram_get_productid();
#else
	char *usbdiskname = "usbdisk";
...
//PHYSICALPATH_FUNC(mod_aicloud_sharelink_physical_handler) {
	...
	mod_aicloud_sharelink_patch_connection(srv, con, p);
	...
	uri_ptr = con->request.uri->ptr;
	uri_len = buffer_string_length(con->request.uri);
	...
	for (k = 0; k < p->conf.alias->used; k++) {
		...
		//Cdbg(1, "uri_ptr=%s", uri_ptr);
		//Cdbg(1, "key=%s, ds=%s, alias_len=%d", ds->key->ptr, ds->value->ptr, alias_len);
		...
		if (0 == (con->conf.force_lowercase_filenames ?
			...
			//Cdbg(1, "44444444444444444444444, con->physical.path=%s, con->physical.basedir=%s", con->physical.path->ptr, con->physical.basedir->ptr);
			break;
		}
	}
#endif	
	//- share_link_type: 0: none, 1: sharelink for general use, 2: sharelink for router sync
	if(con->share_link_type != 0 && con->share_link_type != 1)  return HANDLER_GO_ON;
	...
	Cdbg(DBE, "con->mode=%d", con->mode);
	Cdbg(DBE, "con->physical.path->ptr=%s", con->physical.path->ptr);
	Cdbg(DBE, "con->request.uri->ptr=%s", con->request.uri->ptr);
	Cdbg(DBE, "p->mnt_path->ptr=%s", p->mnt_path->ptr);
	
	if( con->mode == DIRECT && 
		p->mnt_path != NULL &&
		strncmp( con->physical.path->ptr, p->mnt_path->ptr, p->mnt_path->used-1) == 0 ){
...
#ifndef APP_IPKG
int mod_aicloud_sharelink_plugin_init(plugin *p);
int mod_aicloud_sharelink_plugin_init(plugin *p) {
	p->version     = LIGHTTPD_VERSION_ID;
	p->name        = buffer_init_string("mod_aicloud_sharelink");

	p->init        = mod_aicloud_sharelink_init;
	p->set_defaults = mod_aicloud_sharelink_set_defaults;
	p->handle_physical = mod_aicloud_sharelink_physical_handler;
	p->cleanup     = mod_aicloud_sharelink_free;
...
#else
int aicloud_mod_aicloud_sharelink_plugin_init(plugin *p);
int aicloud_mod_aicloud_sharelink_plugin_init(plugin *p) {
	p->version     = LIGHTTPD_VERSION_ID;
	p->name        = buffer_init_string("mod_aicloud_sharelink");
