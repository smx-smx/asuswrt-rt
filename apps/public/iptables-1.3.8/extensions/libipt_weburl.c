...
static struct option opts[] = 
{
	...
	{ .name = "contains_nocase", 		.has_arg = 1, .flag = 0, .val = WEBURL_CONTAINS_NOCASE_TYPE },	//string, no case sensitive
	{ .name = 0 }
};
...
static void final_check(unsigned int flags)
{
	if (flags %10 == 0)
	{
		param_problem_exit_error("You must specify '--contains' or '--contains_regex' or '--matches_exactly' or '--contains_nocase'");
	}
}
