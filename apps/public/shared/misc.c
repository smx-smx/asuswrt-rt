...
{
	return (find_word(nvram_safe_get((char *)key), word) != NULL);
}

int nvram_is_empty(const char *key)
{
	char *p;
	return (((p = nvram_get((char *)key)) == NULL) || (*p == 0));
}
...
static uint32_t crc_table[256]={ 0x00000000,0x77073096,0xEE0E612C,0x990951BA,
