...
#else
		snprintf(tmp, 100, "mkdir -p %s", PPP_DIR);
		system(tmp);
#endif
...
#else
	char modem_enable[64], user[64], pass[64], isp[64], baud[64];
	snprintf(modem_enable, 64, "%s", nvram_safe_get("modem_enable"));
	snprintf(isp, 64, "%s", nvram_safe_get("modem_isp"));
	snprintf(user, 64, "%s", nvram_safe_get("modem_user"));
	snprintf(pass, 64, "%s", nvram_safe_get("modem_pass"));
	snprintf(baud, 64, "%s", nvram_safe_get("modem_baud"));
#endif
...
int write_beceem_conf(const char *eth_node){
	...
	char *user = nvram_safe_get("modem_user");
	char *pass = nvram_safe_get("modem_pass");
	char *ttlsid = nvram_safe_get("modem_baud");
#else
	char modem_enable[64], user[64], pass[64], isp[64], ttlsid[64];
	snprintf(modem_enable, 64, "%s", nvram_safe_get("modem_enable"));
	snprintf(isp, 64, "%s", nvram_safe_get("modem_isp"));
	snprintf(user, 64, "%s", nvram_safe_get("modem_user"));
	snprintf(pass, 64, "%s", nvram_safe_get("modem_pass"));
	snprintf(ttlsid, 64, "%s", nvram_safe_get("modem_ttlsid"));
#endif
...
int asus_sd(const char *device_name, const char *action){
	...
	if(!check_hotplug_action(action)){
		...
		eval("hotplug", "block");
#else
		snprintf(tmp, 100, "hotplug block");
		system(tmp);
#endif
...
after_change_xhcimode:
	...
	if(!isdigit(*ptr)){
		...
		if(!strcmp(nvram_safe_get(prefix), "storage")){
			nvram_set(strcat_r(prefix, "_act", tmp), (char *)device_name);
		}
	}
...
int asus_sg(const char *device_name, const char *action){
	...
	if(!strcmp(nvram_safe_get("beceem_switch"), "1")
		...
		xstart("switchmode");
#else
		snprintf(modem_cmd, 128, "switchmode &");
		system(modem_cmd);
#endif
	}
	else
	if(is_gct_dongle(0, vid, pid)){
		if(strcmp(nvram_safe_get("stop_sg_remove"), "1")){
			...
			xstart("gctwimax", "-D");
#else
			snprintf(modem_cmd, 128, "gctwimax -D &");
			system(modem_cmd);
#endif
		}
	}
...
#else
			snprintf(modem_cmd, 128, "usb_modeswitch -c %s &", switch_file);
			system(modem_cmd);
#endif
		}
	}
...
int asus_sr(const char *device_name, const char *action){
	...
	if(is_gct_dongle(0, vid, pid)){
		if(strcmp(nvram_safe_get("stop_cd_remove"), "1")){
			...
			xstart("gctwimax", "-D");
#else
			snprintf(eject_cmd, 128, "gctwimax -D &");
			system(eject_cmd);
#endif
		}
	}
	...
	if(is_storage_cd(0, vid, pid)){
...
#else
		snprintf(eject_cmd, 128, "sdparm --command=eject /dev/%s", device_name);
		system(eject_cmd);
#endif
		sleep(1);
		...
		if(find_sg_of_device(device_name, sg_device, sizeof(sg_device)) != NULL){
			...
			eval("sdparm", "--command=eject", eject_cmd);
#else
			snprintf(eject_cmd, 128, "sdparm --command=eject /dev/%s", sg_device);
			system(eject_cmd);
#endif
			sleep(1);
		}
	...
	if(!check_hotplug_action(action)){
		...
		if(!strcmp(current_act, usb_node)){
			...
			while(hadOptionModule() && retry < 3){
				while (getOptionModule_refcnt() > 0 && refcnt_retry < 10) {
					usleep(300000);	//check reference count every 300ms
					refcnt_retry++;
				}
...
int asus_usb_interface(const char *device_name, const char *action){
...
#else
				snprintf(modem_cmd, 128, "usb_modeswitch -c %s.%s &", USB_MODESWITCH_CONF, port_path);
				system(modem_cmd);
#endif
			}
	...
	if(is_beceem_dongle(1, vid, pid)){
		...
		if(!strcmp(isp, "Yota")){
...
#else
			snprintf(modem_cmd, 128, "ln -sf /tmp/Beceem_firmware/macxvi200.bin.normal /tmp/Beceem_firmware/macxvi200.bin");
			system(modem_cmd);
			snprintf(modem_cmd, 128, "ln -sf /tmp/Beceem_firmware/macxvi.cfg.yota /tmp/Beceem_firmware/macxvi.cfg");
			system(modem_cmd);
#endif
		}
		else if(!strcmp(isp, "GMC")){
			...
			snprintf(modem_cmd, 128, "ln -sf /tmp/Beceem_firmware/macxvi200.bin.normal /tmp/Beceem_firmware/macxvi200.bin");
			system(modem_cmd);
			snprintf(modem_cmd, 128, "ln -sf /tmp/Beceem_firmware/macxvi.cfg.gmc /tmp/Beceem_firmware/macxvi.cfg");
			system(modem_cmd);
#endif
		}
		else if(!strcmp(isp, "FreshTel")){
			...
			snprintf(modem_cmd, 128, "ln -sf /tmp/Beceem_firmware/macxvi200.bin.normal /tmp/Beceem_firmware/macxvi200.bin");
			system(modem_cmd);
			snprintf(modem_cmd, 128, "ln -sf /tmp/Beceem_firmware/macxvi.cfg.freshtel /tmp/Beceem_firmware/macxvi.cfg");
			system(modem_cmd);
#endif
		}
		else if(!strcmp(isp, "Giraffe")){
...
#else
			snprintf(modem_cmd, 128, "ln -sf /tmp/Beceem_firmware/macxvi200.bin.giraffe /tmp/Beceem_firmware/macxvi200.bin");
			system(modem_cmd);
			snprintf(modem_cmd, 128, "ln -sf /tmp/Beceem_firmware/macxvi.cfg.giraffe /tmp/Beceem_firmware/macxvi.cfg");
			system(modem_cmd);
#endif
		}
		else{
...
#if !defined(RT4GAC55U)	//skip other 3G and 4G dongle
...
#else
		snprintf(modem_cmd, 128, "insmod /lib/modules/usbhost/option.ko vendor=0x%04x product=0x%04x", vid, pid);
		system(modem_cmd);
#endif
		sleep(1);
	}
