...
if [ ! -f "$APPS_INSTALL_PATH/bin/ipkg" ] || [ -z "$had_uclibc" ]; then
	...
	if [ -z "$apps_from_internet" ]; then
		...
		#echo "wget --spider $wget_options $dl_path/$base_file"		#removed, not support --spider in current busybox version.
		#wget --spider $wget_options $dl_path/$base_file
		#if [ "$?" != "0" ]; then
		#	echo "There is no $base_file from Internet!"
		#	tcapi set Apps_Entry apps_state_errmsg "There is no $base_file from Internet!"
		...
		if [ -z "$wget_pid" ] || [ $wget_pid -lt 1 ]; then
			...
			tcapi set Apps_Entry apps_state_errmsg "Failed to get $base_file from Internet!"
			tcapi set Apps_Entry apps_state_error 6
			return 1
		fi
		i=0
		...
		if [ -z "$percent" ] || [ $percent -ne 100 ]; then
			...
			tcapi set Apps_Entry apps_state_errmsg "Couldn't complete to download $base_file from Internet!"
			tcapi set Apps_Entry apps_state_error 6
			return 1
		fi
