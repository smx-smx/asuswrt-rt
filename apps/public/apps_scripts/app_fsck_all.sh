...
port_num=`tcapi get USB_Entry ehci_ports |awk '{print NF}'`
...
while [ $p -le $port_num ]; do
	usb_path=usb_path$p
	nvram=${usb_path}_act
	nvram_value=`tcapi get USB_Entry $nvram`
...
set _ pipefail#set -o pipefail
tcapi set USB_Entry ${usb_path}_fs_path${i}_error 2
...
if [ ${RET} -ne 0 ]; then
	tcapi set USB_Entry ${usb_path}_fs_path${i}_error 1
else
	tcapi set USB_Entry ${usb_path}_fs_path${i}_error 0
fi
