...
int get_tokenfile_info()
{
        ...
        for(i = 0; i < info->num; i++)
        {
                ...
                snprintf(tokenfile, sizeof(char)*(strlen(info->paths[i]) + 24), "%s/.usbclient_tokenfile", info->paths[i]);
                DEBUG("tokenfile = %s\n", tokenfile);
...
int write_to_tokenfile(char *mpath)
{
        ...
        snprintf(filename, sizeof(char)*(strlen(mpath) + 24), "%s/.usbclient_tokenfile", mpath);
