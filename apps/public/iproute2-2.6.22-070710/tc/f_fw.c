...
#ifdef TC_LINUX
	fprintf(stderr, "\nNode: CLASSID is parsed as hexadecimal input.\n");
#endif
}
...
static int fw_parse_opt(struct filter_util *qu, char *handle, int argc, char **argv, struct nlmsghdr *n)
{
	...
	if (handle) {		
		...
		if ( (slash = strchr(handle, '/')) != NULL )
			*slash = '\0';
			...
			fprintf(stderr, "else handle is %s\n", handle);
		}
		
		if ( get_u32(&t->tcm_handle, handle, 0) ) {
			fprintf(stderr, "t->tcm_handle is %u\n", t->tcm_handle);
	...
	while (argc > 0) {
		if (matches(*argv, "classid") == 0 ||
			...
			/*move to here, default mask is 0xFFFFFFFF*/
			addattr32( n, MAX_MSG, TCA_FW_MASK, mask );
...
static int fw_print_opt(struct filter_util *qu, FILE *f, struct rtattr *opt, __u32 handle)
{
	...
	if ( handle || tb[TCA_FW_MASK] ) {
		...
		if ( (tb[TCA_FW_MASK] && (mask = *(__u32 *)RTA_DATA(tb[TCA_FW_MASK]))) != 0xffffffff ) {
			
			//fprintf(stderr, "==>fw_print_opt(): handle xx/xx form.\n");
			fprintf(f, "handle 0x%x/0x%x", mark, mask );
		}
		else {
			
			//fprintf(stderr, "==>fw_print_opt(): handle xx form.\n");
			fprintf(f, "handl 0x%x", handle);
		}
	}
#else
	if (handle)
