...
int get_account_list(int *acc_num, char ***account_list) {
	...
	*acc_num = tcapi_get_int("Samba_Entry", "acc_num");
	if(*acc_num <= 0)
		return 0;

	//for special character, for instance, '#' will be translated to '%23', so we have to multiply '3'.
	//for the acc_list, it contains username and password, so we have to multiply '2'.
	...
	if(tcapi_get("Samba_Entry", "acc_list", nv)) {
...
extern int initial_var_file(const char *const account, const char *const mount_path) {
	...
	if(account == NULL // share mode.
		|| !strcmp(account, tcapi_get_string("Account_Entry0", "username", buf))//|| !strcmp(account, nvram_safe_get("http_username"))
			){
...
extern int add_account(const char *const account, const char *const password){
	...
	tcapi_get("Samba_Entry", "acc_list", nvram_value);
	len = strlen(nvram_value);
	if(len > 0){
		ptr = nvram_value+len;
		sprintf(ptr, "<%s>%s", ascii_user, ascii_passwd);
		tcapi_set("Samba_Entry", "acc_list", nvram_value);//nvram_set("acc_list", nvram_value);

		sprintf(nvram_value, "%d", acc_num+1);
		tcapi_set("Samba_Entry", "acc_num", nvram_value);//nvram_set("acc_num", nvram_value);
	}
	else{
		sprintf(nvram_value, "%s>%s", ascii_user, ascii_passwd);
		tcapi_set("Samba_Entry", "acc_list", nvram_value);//nvram_set("acc_list", nvram_value);

		tcapi_set("Samba_Entry", "acc_num", "1");//nvram_set("acc_num", "1");
	}

	// spinlock_lock(SPINLOCK_NVRAMCommit);
	// nvram_commit();
	// spinlock_unlock(SPINLOCK_NVRAMCommit);
	tcapi_set("GUITemp_Entry0", "action_script", "restart_ftpsamba");
...
extern int del_account(const char *const account){
	...
	if(nv && strlen(nv) > 0){
		while((b = strsep(&nvp, "<")) != NULL){
			...
			if(!strcmp(account, char_user)){
				if(--acc_num == 0){
					tcapi_set("Samba_Entry", "acc_num", "0");//nvram_set("acc_num", "0");
					tcapi_set("Samba_Entry", "acc_list", "");//nvram_set("acc_list", "");
	...
	if(i <= 0){
		tcapi_set("Samba_Entry", "st_samba_mode", "1");//nvram_set("st_samba_mode", "1");
...
extern int add_folder(const char *const account, const char *const mount_path, const char *const folder) {
	...
	for (i = -1; i < acc_num; ++i) {
		...
		if(i == -1 // share mode.
			|| !strcmp(account_list[i], tcapi_get_string("Account_Entry0", "username", buf))// || !strcmp(account_list[i], nvram_safe_get("http_username"))
				){
			samba_right = DEFAULT_SAMBA_RIGHT;
			ftp_right = DEFAULT_FTP_RIGHT;
			dms_right = DEFAULT_DMS_RIGHT;
