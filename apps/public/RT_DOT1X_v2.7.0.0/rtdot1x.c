...
static void usage(void)
{
	...
	DBGPRINT(RT_DEBUG_OFF, "-i <card_number> : iNIC indicates which card is used\n");
	DBGPRINT(RT_DEBUG_OFF, "-n <card_number> : non-iNIC indicates which card is used\n");
...
int main(int argc, char *argv[])
{
	...
	FILE *file;
	char tmpstr[64], tmpstr_rm[64];
	strcpy(prefix_name, "ra");
	int dual_if = 0;

	for (;;)
	{
		c = getopt(argc, argv, "d:i:n:h");
		if (c < 0)
			break;
		...
		switch (c)
		{
			...
			case 'i': 
				...
				dual_if = 1;
				sprintf(tmpstr, "/var/run/rtdot1xd_%d.pid", (int)strtol(optarg, 0, 10) );
				file= fopen(tmpstr, "w");
			    break;
			...
			case 'n': 
				...
				if((int)strtol(optarg, 0, 10) == 1){
					//ra, do nothing.
				}
				else if((int)strtol(optarg, 0, 10) == 2){
					//ra to be rai
					sprintf(prefix_name, "%si", prefix_name);
				}
				else{
	...
	if(dual_if == 0){
		sprintf(tmpstr, "/var/run/rtdot1xd.pid");
		file= fopen(tmpstr, "w");
	}
