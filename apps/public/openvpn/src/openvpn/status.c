...
void update_nvram_status(int event)
{
	...
	case EVENT_AUTH_FAILED:
		snprintf(cmd, sizeof(cmd), "tcapi set OpenVPN_Entry%d errno %d", unit, ERRNO_AUTH);
		system(cmd);
		snprintf(cmd, sizeof(cmd), "tcapi set OpenVPN_Entry%d state %d", unit, ST_ERROR);
		system(cmd);
		break;
	case EVENT_TLS_ERROR:
		snprintf(cmd, sizeof(cmd), "tcapi set OpenVPN_Entry%d errno %d", unit, ERRNO_SSL);
		system(cmd);
		snprintf(cmd, sizeof(cmd), "tcapi set OpenVPN_Entry%d state %d", unit, ST_ERROR);
		system(cmd);
		break;
