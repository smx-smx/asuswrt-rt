...
/* $Id: //BBN_Linux/Branch/Branch_for_SDK_Release_MultiChip_20111013/tclinux_phoenix/apps/public/boa-asp/src/buffer.c#2 $ */
...
/*
 * Name: req_write_escape_http_new
 * Description: Buffers and "escapes" data before sending to client.
 *  as above, but translates as it copies, into a form suitably
 *  encoded for URLs in HTTP headers.
 * Returns: -1 for error, otherwise how much is stored
...
#ifdef INET6
	 /* In request url, the ipv6 ip address will be enclosed in the square brackets '[' and ']'. And this square brackets should not be transferred. 
	 So we just copy the square brackets to dest string*/
	ptr = strstr(inp, LEFT_SQUARE_BRACKET);
	if(ptr != NULL) {
		if (strstr(ptr, RIGHT_SQUARE_BRACKET) != NULL) {
    			while (((c = *inp++) != ']') && left > 0) {
            			*dest++ = c;
            			left--;
    			}
				
			//copy the right square bracket ']'
			if(left > 0) {
    				*dest++ = c;
    				left--;
			}
			else{
        			fprintf(stderr, "Ran out of the req Buffer space!\n");
        			req->status = DEAD;
        			return -1;
    			}				
		}
		else {
			fprintf(stderr, "URL format error! The right square bracket miss.\n");
			req->status = DEAD;
			return -1;
		}
	}
...
#endif
	{
        bytes_written = write(req->fd, req->buffer + req->buffer_start,bytes_to_write);
	}
#if defined(TCSUPPORT_WEBSERVER_SSL)
	else{
