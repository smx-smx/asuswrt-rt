...
/* $Id: //BBN_Linux/Branch/Branch_for_SDK_Release_MultiChip_20111013/tclinux_phoenix/apps/public/boa-asp/src/get.c#2 $*/
...
int init_get(request * req)
{
	...
	//Sam
	//add for preprocess tcWebAPI in .js file
	if(strstr(req->pathname,".js"))
...
#endif
	{		
        bytes_written = write(req->fd, req->data_mem + req->filepos,bytes_to_write);
	}
#if defined(TCSUPPORT_WEBSERVER_SSL)
	else{
