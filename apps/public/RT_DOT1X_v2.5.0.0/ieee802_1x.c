...
static void ieee802_1x_send(rtapd *rtapd, struct sta_info *sta, u8 type, u8 *data, size_t datalen)
{
	...
	if (sta->ethertype == ETH_P_PRE_AUTH)
	{
        if (RT_ioctl(rtapd->ioctl_sock, RT_PRIV_IOCTL, (void *)&bSyncmiimac, sizeof(bSyncmiimac), rtapd->prefix_wlan_name, sta->ApIdx, OID_SYNCMIIMAC))
	    {
	    	DBGPRINT(RT_DEBUG_TRACE,"ioctl Get Syncmiimac failed, Not MII interface\n");
	    ...
	    {
			DBGPRINT(RT_DEBUG_TRACE,"ioctl Get Syncmiimac=%d\n", bSyncmiimac);
			if (bSyncmiimac)
			{
		if (send(sta->SockNum/*rtapd->eth_sock*/, buf, len, 0) < 0)
			perror("send[WPA2 pre-auth]");
	...
	{
				if (RT_ioctl(rtapd->ioctl_sock, RT_PRIV_IOCTL, buf, len, rtapd->prefix_wlan_name, sta->ApIdx, RT_OID_SEND_PRE_AUTH))
				{
					DBGPRINT(RT_DEBUG_ERROR,"ioctl Send Pre-Auth Packet failed for ieee802_1x_send(len=%d)\n", len);
				}
				else
				{
					DBGPRINT(RT_DEBUG_TRACE,"ioctl Send Pre-Auth Packet for ieee802_1x_send(len=%d)\n", len);
				}
			}
	    }
    }
