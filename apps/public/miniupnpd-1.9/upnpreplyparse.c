...
NameValueParserEndElt(void * d, const char * name, int l)
{
	...
	if(strcmp(data->curelt, "NewPortListing") != 0)
	{
		...
		data->l_head = nv;
                ++data->totalItem;	//andy chiu add 2014/06/20
	}
	data->cdata = NULL;
	data->cdatalen = 0;
	data->topelt = 0;
               ...
               struct NameValueParserData * data)
{
	struct xmlparser parser;
        memset(&parser, 0, sizeof(struct xmlparser));   // Andy Chiu
        memset(data, 0, sizeof(struct NameValueParserData)); // Andy Chiu
	data->l_head = NULL;
                          ...
                          const char * Name)
{
        ...
        nv = nv->l_next)
    {
        ...
        {
            p = nv->value;
            //printf("$$$$$GetValueFromNameValueList():Name=<%s> value=<%s>\n", Name, p);  // Andy Chiu
            break; // Andy Chiu
        }
    }
    return p;
