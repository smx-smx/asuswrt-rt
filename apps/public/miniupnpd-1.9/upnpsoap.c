...
GetConnectionTypeInfo(struct upnphttp * h, const char * action)
{
	...
	int i;
	int flag = 0;
	struct NameValue *nv = NULL;
	
	//+++ Andy Chiu
	struct NameValueParserData data1; // Andy Chiu
	ParseNameValue(h->req_buf + h->req_contentoff, h->req_contentlen, &data1);

	//Andy Chiu, 2014/11/14.
	//check all arguments
	nv = data1.l_head;
	for(i = 0; i < data1.totalItem && nv; ++i)
	{
		if(!strcmp("GetConnectionTypeInfo", nv->name))
			flag = 1;
		nv = nv->l_next;
	}

	if((data1.totalItem - flag) > 0)	//there are dumy arguments
	{	
	    ClearNameValueList(&data1);
...
GetTotalBytesSent(struct upnphttp * h, const char * action)
{
	...
	for(i = 0; i < data1.totalItem && nv; ++i)
	{
		if(!strcmp("GetTotalBytesSent", nv->name))
			flag = 1;
		nv = nv->l_next;
	}
...
GetTotalBytesReceived(struct upnphttp * h, const char * action)
{
	...
	for(i = 0; i < data1.totalItem && nv; ++i)
	{
		if(!strcmp("GetTotalBytesReceived", nv->name))
			flag = 1;
		nv = nv->l_next;
	}
...
GetTotalPacketsSent(struct upnphttp * h, const char * action)
{
	...
	for(i = 0; i < data1.totalItem && nv; ++i)
	{
		if(!strcmp("GetTotalPacketsSent", nv->name))
			flag = 1;
		nv = nv->l_next;
	}
...
GetTotalPacketsReceived(struct upnphttp * h, const char * action)
{
	...
	for(i = 0; i < data1.totalItem && nv; ++i)
	{
		if(!strcmp("GetTotalPacketsReceived", nv->name))
			flag = 1;
		nv = nv->l_next;
	}
...
GetCommonLinkProperties(struct upnphttp * h, const char * action)
{
	...
	static const char resp[] =
        ...
        /*"<NewWANAccessType>Cable</NewWANAccessType>"  Andy Chiu  */
        "<NewWANAccessType>Wireless</NewWANAccessType>"  /*  Andy Chiu: Andy, you need to modify this function for multi WAN access  */
	...
	for(i = 0; i < data1.totalItem && nv; ++i)
	{
		if(!strcmp("GetCommonLinkProperties", nv->name))
			flag = 1;
		nv = nv->l_next;
	}
...
GetStatusInfo(struct upnphttp * h, const char * action)
{
	static const char resp[] =
		...
		"</u:%sResponse>";
	struct NameValueParserData data;	//andy chiu 2013/10/3
	...
	//check all arguments
	nv = data.l_head;
	for(i = 0; i < data.totalItem && nv; ++i)
	{
		if(!strcmp("GetStatusInfo", nv->name))
			flag = 1;
		nv = nv->l_next;
	}

	if((data.totalItem - flag) > 0)	//there are dumy arguments
	{	
...
GetNATRSIPStatus(struct upnphttp * h, const char * action)
{
	...
	for(i = 0; i < data1.totalItem && nv; ++i)
	{
		if(!strcmp("GetNATRSIPStatus", nv->name))
			flag = 1;
		nv = nv->l_next;
	}
...
GetExternalIPAddress(struct upnphttp * h, const char * action)
{
	...
	for(i = 0; i < data.totalItem && nv; ++i)
	{
		if(!strcmp("GetExternalIPAddress", nv->name))
			flag = 1;
		nv = nv->l_next;
	}
...
AddPortMapping(struct upnphttp * h, const char * action)
{
    ...
    char * int_ip=NULL, * int_port=NULL, * ext_port=NULL, * protocol=NULL, * desc=NULL;  // Andy Chiu
    char * leaseduration_str=NULL; // Andy Chiu
	unsigned int leaseduration;
    char * r_host=NULL;  // Andy Chiu
	...
	syslog(LOG_INFO, "%s: ext port %hu to %s:%hu protocol %s for: %s leaseduration=%u rhost=%s",
           action, eport, int_ip, iport, protocol, desc ? desc : "NULL", leaseduration,
           r_host ? r_host : "NULL");  // Andy Chiu
...
AddAnyPortMapping(struct upnphttp * h, const char * action)
{
	...
	struct NameValueParserData data;
    //const char * int_ip, * int_port, * ext_port, * protocol, * desc;  // Andy Chiu
    //const char * r_host;  // Andy Chiu
	unsigned short iport, eport;
    //const char * leaseduration_str;  // Andy Chiu
	unsigned int leaseduration;
...
GetSpecificPortMappingEntry(struct upnphttp * h, const char * action)
{
	...
	int bodylen;
	struct NameValueParserData data;
    //const char * r_host, * ext_port, * protocol;  // Andy Chiu
	unsigned short eport, iport;
	char int_ip[32];
	char desc[64];
	unsigned int leaseduration = 0;
    char * r_host=NULL, * ext_port=NULL, * protocol=NULL;  // Andy Chiu
...
DeletePortMapping(struct upnphttp * h, const char * action)
{
	...
	//andy chiu 2014/06/20
#ifdef UPNP_STRICT
	if(!ext_port || !protocol || !r_host || data.totalItem > 3)
#else
	if(!ext_port || !protocol || data.totalItem > 2)
#endif
	{
		ClearNameValueList(&data);
		SoapError(h, 402, "Invalid Args");
...
DeletePortMappingRange(struct upnphttp * h, const char * action)
{
	...
	static const char resp[] =
		...
		"</u:DeletePortMappingRangeResponse>";
	struct NameValueParserData data;
    //const char * protocol;   // Andy Chiu
    //const char * startport_s, * endport_s;  // Andy Chiu
	unsigned short startport, endport;
	/*int manage;*/
	unsigned short * port_list;
	unsigned int i, number = 0;
    char * protocol=NULL;  // Andy Chiu
    char * startport_s=NULL, * endport_s=NULL;  // Andy Chiu
	UNUSED(action);
...
GetListOfPortMappings(struct upnphttp * h, const char * action)
{
	...
	struct NameValueParserData data;
    //const char * startport_s=NULL, * endport_s=NULL, *manage_s=NULL;  // Andy Chiu
	unsigned short startport, endport;
    //const char * protocol=NULL;  // Andy Chiu
	/*int manage;*/
    //const char * number_s;
	int number;
	unsigned short * port_list;
	unsigned int i, list_size = 0;
    //Andy Chiu: oh ... well... wrong type pointer ...
    char * startport_s=NULL, * endport_s=NULL, *manage_s=NULL;  // Andy Chiu
    char * protocol=NULL;  // Andy Chiu
    char * number_s=NULL;  // Andy Chiu
    ...
    //if (startport_s == NULL || endport_s == NULL || protocol == NULL ||
    //        number_s == NULL) // Andy Chiu
    // Andy Chiu: oh...well... I do not know but here we can read cntrl character sometimes ....
    if ((!startport_s) || (!endport_s) || (!protocol) || (!number_s) || (!manage_s))  // Andy Chiu
    {
		SoapError(h, 402, "Invalid Args");
		ClearNameValueList(&data);
		return;
    ...
    {
        if (!isalnum(startport_s[0]) || !isalnum(endport_s[0]) || !isalnum(protocol[0]) || !isalnum(number_s[0]) || !isalnum(manage_s[0]))
        {
            printf("####is NOT an alnum!!!\n");
            SoapError(h, 402, "Invalid Args");
            ClearNameValueList(&data);
            return;
        }
...
// Andy Chiu: Well .... not sure what to deal with ...but I still do something ...
static void
GetDefaultConnectionService(struct upnphttp * h, const char * action)
{
	...
	int flag = 0;
	struct NameValue *nv = NULL;
	struct NameValueParserData data; // Andy Chiu
	ParseNameValue(h->req_buf + h->req_contentoff, h->req_contentlen, &data);
	...
	for(i = 0; i < data.totalItem && nv; ++i)
	{
		if(!strcmp("GetDefaultConnectionService", nv->name))
			flag = 1;
		nv = nv->l_next;
	}
...
#ifdef UPNP_STRICT  // Andy Chiu: needed for compilance tool 3.1.3
    if (!connection_type || connection_type[0] == '\0')
    {
		ClearNameValueList(&data);
		SoapError(h, 402, "Invalid Args");
		return;
...
RequestConnection(struct upnphttp * h, const char * action)
{
	...
	for(i = 0; i < data.totalItem && nv; ++i)
	{
		if(!strcmp("RequestConnection", nv->name))
			flag = 1;
		nv = nv->l_next;
	}
	...
	{	
		...
		//TODO: active wan connection
//		qmi_usock_msg_sendto_appmgr(CM_MOD_ID_UPNP, 0, 0, &force_connect, sizeof(force_connect), MSGID_WAN_CONNECT);
		UNUSED(action);
		BuildSendAndCloseSoapResp(h, resp, sizeof(resp)-1);
    // <<<<<< Andy Chiu
...
ForceTermination(struct upnphttp * h, const char * action)
{
	...
	for(i = 0; i < data.totalItem && nv; ++i)
	{
		if(!strcmp("ForceTermination", nv->name))
			flag = 1;
		nv = nv->l_next;
	}
...
QueryStateVariable(struct upnphttp * h, const char * action)
{
	...
	int bodylen;
	struct NameValueParserData data;
    //const char * var_name;  // Andy Chiu
    char * var_name=NULL;  // Andy Chiu
...
GetFirewallStatus(struct upnphttp * h, const char * action)
{
	...
	for(i = 0; i < data1.totalItem && nv; ++i)
	{
		if(!strcmp("GetFirewallStatus", nv->name))
			flag = 1;
		nv = nv->l_next;
	}
...
AddPinhole(struct upnphttp * h, const char * action)
{
	...
	int bodylen;
	struct NameValueParserData data;
    char * rem_host=NULL, * rem_port=NULL, * int_ip=NULL, * int_port=NULL, * protocol=NULL, * leaseTime=NULL;  // Andy Chiu
	int uid = 0;
    ...
    //+++ Andy Chiu
    if ((!rem_host) || (!rem_port) || (!int_ip) || (!int_port) || (!protocol) || (!leaseTime))
    {
        SoapError(h, 402, "Invalid Args");
        goto clear_and_exit;
    }
...
UpdatePinhole(struct upnphttp * h, const char * action)
{
	static const char resp[] =
		...
		"</u:UpdatePinholeResponse>";
	struct NameValueParserData data;
    const char * uid_str=NULL, * leaseTime=NULL;
	char iaddr[INET6_ADDRSTRLEN];
	unsigned short iport;
	int ltime;
	int uid;
	int n;
    char body[512]; // Andy Chiu
    ...
    //uid = uid_str ? atoi(uid_str) : -1;  // Andy Chiu
    //ltime = leaseTime ? atoi(leaseTime) : -1;  // Andy Chiu
    //+++ Andy Chiu
    if ((uid_str==NULL)  || (leaseTime==NULL))
    {
        SoapError(h, 402, "Invalid Args");
        return;
    }
    ...
    {
        body[0]='\0';
        sprintf(body, "%s", uid_str); // Andy Chiu: borrow body tempary
        if (!isdigit(body[0]))
        {
            SoapError(h, 402, "Invalid Args");
            return;
...
GetOutboundPinholeTimeout(struct upnphttp * h, const char * action)
{
	...
	int bodylen;
	struct NameValueParserData data;
    char * int_ip=NULL, * int_port=NULL, * rem_host=NULL, * rem_port=NULL, * protocol=NULL;  // Andy Chiu
	int opt=0, proto=0;
	unsigned short iport, rport;
	...
	protocol = GetValueFromNameValueList(&data, "Protocol");
    //+++ Andy Chiu
    if ((!int_ip) || (!int_port) || (!rem_host) || (!rem_port) || (!protocol))
    {
        SoapError(h, 402, "Invalid Args");
        return;
    }
	...
	proto = atoi(protocol);
        ...
        // Andy Chiu: I mark this line off because when the error case; this print out crash the program
        //syslog(LOG_INFO, "%s: retrieving timeout for outbound pinhole from [%s]:%hu to [%s]:%hu protocol %s", action, int_ip, iport,rem_host, rport, protocol);
    }
    //--- Andy Chiu
	/* TODO */
    r = -1;/*upnp_check_outbound_pinhole(proto, &opt);*/  // Andy Chiu: Why not???
    //r = upnp_check_outbound_pinhole(proto, &opt);  // Andy Chiu: why not???
	switch(r)
	{
		case 1:	/* success */
			bodylen = snprintf(body, sizeof(body), resp,
...
CheckPinholeWorking(struct upnphttp * h, const char * action)
{
	...
	int r;
	struct NameValueParserData data;
    //const char * uid_str;  // Andy Chiu
	int uid;
	char iaddr[INET6_ADDRSTRLEN];
	unsigned short iport;
	unsigned int packets;
    char * uid_str=NULL;  // Andy Chiu
	if(CheckStatus(h)==0)
		return;
...
//		"<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
	...
	// >>>>>> Andy chiu 2014/06/25
	struct NameValueParserData data;	//andy chiu 2014/06/25
	int i;
	int flag = 0;
	struct NameValue *nv = NULL;
	ParseNameValue(h->req_buf + h->req_contentoff, h->req_contentlen, &data);
	...
	for(i = 0; i < data.totalItem && nv; ++i)
	{
		if(!strcmp("GetSupportedProtocols", nv->name))
			flag = 1;
		nv = nv->l_next;
	}
...
GetAssignedRoles(struct upnphttp * h, const char * action)
{
	...
	for(i = 0; i < data.totalItem && nv; ++i)
	{
		if(!strcmp("GetAssignedRoles", nv->name))
			flag = 1;
		nv = nv->l_next;
	}
...
ExecuteSoapAction(struct upnphttp * h, const char * action, int n)
{
	...
	if(p)
	{
		...
		//syslog(LOG_INFO, "$#$#$#$#$#-->SoapMethod return 401-> 602\n"); // Andy Chiu
    //SoapError(h, 401, "Invalid Action");  // Andy Chiu
    SoapError(h, 602, "Optional Action Not Implemented");  // Andy Chiu: change error code for those not implmented soapmethod
}
