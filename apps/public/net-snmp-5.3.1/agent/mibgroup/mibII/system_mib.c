...
#include "../../../../lib/libtcapi.h"
	 ...
	 *
	 *********************/
#define SYS_VERS_DESC	"Ralink ADSL Modem Software Version %s"
...
#ifdef TRENDCHIP
	tcapi_get("DeviceInfo","FwVer", version_info);
	snprintf(version_descr, sizeof(version_descr), SYS_VERS_DESC, version_info);
#else
#ifdef HAVE_UNAME
...
#ifdef TRENDCHIP
#if 0
    tcapi_get("Snmpd_Entry","sysLocation",sysLocation);
	tcapi_get("Snmpd_Entry","sysContact",sysContact);
#endif
	snmpdfp=fopen(snmp_config_path, "r");
	if(snmpdfp){
		fgets(sysLocation, 256, snmpdfp);
		sysLocation[strlen(sysLocation)-1]='\0';
		fgets(sysContact, 256, snmpdfp);
		sysContact[strlen(sysContact)-1]='\0';
		fgets(sysName, 256, snmpdfp);
		sysName[strlen(sysName)-1]='\0';
		fclose(snmpdfp);
		//unlink(snmp_config_path);
	}
#endif
    /*
            ...
            u_char * statP, oid * name, size_t name_len)
{
    ...
    case ACTION:               /* Perform the SET action (if reversible) */
        ...
        switch ((char) name[7]) {
        case SYSCONTACT:
        	tcapi_set("Snmpd_Entry","sysContact",buf);
        	break;
        case SYSTEMNAME:
        	tcapi_set("Snmpd_Entry","sysName",buf);
        	break;
        case SYSLOCATION:
        	tcapi_set("Snmpd_Entry","sysLocation",buf);
        	break;
        default:
        	break;
        }     
