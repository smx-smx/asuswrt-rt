                       ...
                       size_t * var_len, WriteMethod ** write_method)
{
    ...
    case VERDATE:
        sprintf(errmsg, "$Date: 2010/10/01 $");
        *var_len = strlen(errmsg);
        return ((u_char *) errmsg);
    case VERCDATE:
        curtime = time(NULL);
    ...
    case VERIDENT:
        sprintf(errmsg,
                "$Id: //BBN_Linux/Branch/Branch_for_SDK_Release_MultiChip_20111013/tclinux_phoenix/apps/public/net-snmp-5.3.1/agent/mibgroup/ucd-snmp/versioninfo.c#1 $");
        *var_len = strlen(errmsg);
        return ((u_char *) errmsg);
    case VERCONFIG:
#ifdef CONFIGURE_OPTIONS
