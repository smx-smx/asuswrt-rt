...
#define INFO_ADSL	"Info_Adsl"
#define ADSL_STATS	"lineState"
#define UP_STATUS	"up"
#define DOWN_STATUS	"down"
#define ADSL_UP		0
#define ADSL_DOWN 	-1
...
sendstartTrap(void)
{
	...
	tcapi_get(SNMP_NODE, SNMP_COLDTRAP, trapFlag);
#if defined(TCSUPPORT_START_TRAP)
	if(strcmp(trapFlag, TRAP_FLAG) == 0) {
		//tcdbg_printf("\n==>send warmstrat trap\n");
		send_easy_trap(SNMP_TRAP_WARMSTART, 0);
		tcapi_set(SNMP_NODE, SNMP_COLDTRAP, TRAP_CODETRAP_FLAG);
		tcapi_save();
	}
	else {
		//tcdbg_printf("\n==>send coldstrat trap\n");
	...
	if(strcmp(trapFlag, TRAP_FLAG) == 0) {
		...
		tcapi_set(SNMP_NODE, SNMP_COLDTRAP, TRAP_FLAG);
		tcapi_save();
	}
#endif
}
...
int 
check_adsl_status(void)
{
	char status[32] = {0};
	
	tcapi_get(INFO_ADSL, ADSL_STATS, status);

	if(strcmp(status, UP_STATUS) == 0) {
		return ADSL_UP;
		send_easy_trap(SNMP_TRAP_LINKUP, 0);
	}
	else if(strcmp(status, DOWN_STATUS) == 0) {
		return ADSL_DOWN;
	}
	else
		tcdbg_printf("\n==>Invalid adsl status\n");
...
void
sendLinkUpDownTrap(void)
{
	int adsl_status = -1;

	adsl_status = check_adsl_status();
	
	if(adsl_status == ADSL_UP){
		//tcdbg_printf("\n==>adsl status change to up, send linkup trap\n");
		send_easy_trap(SNMP_TRAP_LINKUP, 0);
	}
	else {
		//tcdbg_printf("\n==>adsl status change to up, send linkdown trap\n");
  	       send_easy_trap(SNMP_TRAP_LINKDOWN, 0);
	}
}

RETSIGTYPE
SnmpdLinkStatusChange(int a)
{
	sendLinkUpDownTrap();
}
#endif
...
#if defined(TCSUPPORT_SNMP_FULL)
    /*register signal to send link up/down trap*/
    DEBUGMSGTL(("signal", "registering SIGUSR2 signal handler\n"));
    signal(SIGUSR2, SnmpdLinkStatusChange);
#endif
