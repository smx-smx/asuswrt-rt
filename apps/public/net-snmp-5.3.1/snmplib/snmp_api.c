...
#ifdef TCSUPPORT_SNMP_TRUSTIP		
	if( (pdu->command ==SNMP_MSG_SET) && (is_trustip != 1) ){
		return SNMPERR_TIMEOUT;
	}
#endif
    return SNMPERR_SUCCESS;
}                               /* end snmpv3_parse() */
...
_sess_read(void *sessp, fd_set * fdset)
{
	...
	#ifdef TCSUPPORT_SNMP_TRUSTIP
	{
		...
		char buf[50];
		int ret1, ret2;
		unsigned int trust_start_ip = 0, trust_end_ip = 0;
#if defined(TCSUPPORT_IPV6_SNMP)
		struct sockaddr_in6 *fromv6;
		struct in6_addr trust_start_ipv6,trust_end_ipv6;  
#endif
		...
		if (from != NULL ) {
			...
			/* check trust ip */
			if( ((struct sockaddr_storage*)opaque)->ss_family == AF_INET){
				from = (struct sockaddr_in *)opaque;	
				ret1 = tcapi_get("Snmpd_Entry", "trustStartIP", buf);
				if (ret1 == 0)
					inet_aton(buf, &trust_start_ip);
				ret2 = tcapi_get("Snmpd_Entry", "trustEndIP", buf);
				if (ret2 == 0)
					inet_aton(buf, &trust_end_ip);

				/* only there is trust ip setting */
				if (ret1 == 0 && ret2 == 0)
					if ((unsigned int)from->sin_addr.s_addr > trust_end_ip || 
						(unsigned int)from->sin_addr.s_addr < trust_start_ip) {
						is_trustip = 0;
					}
			}
#if defined(TCSUPPORT_IPV6_SNMP)
			else if( ((struct sockaddr_storage*)opaque) ->ss_family == AF_INET6){ 
				fromv6= (struct sockaddr_in6 *)opaque;
				ret1 = tcapi_get("Snmpd_Entry", "trustStartIPv6", buf);
				if (ret1 == 0)
					inet_pton(AF_INET6,buf,(void*)&trust_start_ipv6);
				ret2 = tcapi_get("Snmpd_Entry", "trustEndIPv6", buf);
				if (ret2 == 0)
					inet_pton(AF_INET6,buf, (void*)&trust_end_ipv6);//
					
				/* only there is trust ip setting */
				if (ret1 == 0 && ret2 == 0) 
					if( memcmp(&(fromv6->sin6_addr),&trust_end_ipv6,sizeof(trust_end_ipv6)) >0 ||
						memcmp(&(fromv6->sin6_addr),&trust_start_ipv6,sizeof(trust_start_ipv6)) <0 ){
						is_trustip = 0;
					}	
			}
#endif
