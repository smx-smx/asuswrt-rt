...
#ifdef TCSUPPORT_PADT_BEFORE_PADI
	if(strlen(ses->name) > 0){
		memset(tmp, 0, sizeof(tmp));
		sprintf(tmp, "%u:%u:%u:%u:%u:%u:%02x:%02x:%02x:%02x:%02x:%02x:%d"
				,ses->remote.sll_family
				,ses->remote.sll_protocol
				,ses->remote.sll_ifindex
				,ses->remote.sll_hatype
				,ses->remote.sll_pkttype
				,ses->remote.sll_halen
