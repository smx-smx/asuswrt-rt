...
bool starter_netkey_init(void)
{
	...
	if (stat(PROC_NETKEY, &stb) != 0)
	{
		...
		{
			ignore_result(system("modprobe -v af_key"));
		}
	...
	if (stat(PROC_MODULES, &stb) == 0)
	{
		ignore_result(system("modprobe -v ah4"));
		ignore_result(system("modprobe -v esp4"));
		ignore_result(system("modprobe -v ipcomp"));
		ignore_result(system("modprobe -v xfrm4_tunnel"));
		ignore_result(system("modprobe -v xfrm_user"));
	}
