...
if [ "$cleanup" = "1" ] && [ -e "$prefix/bin/busybox-new" ]; then
	inode=`ls -i "$prefix/bin/busybox-new" | awk '{print $1}'`
	sub_shell_it=`
		cd "$prefix"
...
rm -f "$prefix/bin/busybox-new" || exit 1
mkdir -p "$prefix/bin" || exit 1
install -m 755 busybox "$prefix/bin/busybox-new" || exit 1
...
for i in $h; do
	...
	if [ "$scriptwrapper" = "y" ]; then
		if [ "$swrapall" != "y" ] && [ "$i" = "/bin/sh" ]; then
			ln $linkopts busybox-new "$prefix/$i" || exit 1
		else
			rm -f "$prefix/$i"
			echo "#!/bin/busybox-new" >"$prefix/$i"
			chmod +x "$prefix/$i"
		fi
		echo "	$prefix/$i"
	else
		if [ "$2" = "--hardlinks" ]; then
			bb_path="$prefix/bin/busybox-new"
		else
			case "$appdir" in
			/)
				bb_path="bin/busybox-new"
			;;
			/bin)
				bb_path="busybox-new"
			;;
