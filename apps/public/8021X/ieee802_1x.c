...
static void ieee802_1x_encapsulate_radius(rtapd *rtapd, struct sta_info *sta, u8 *eap, size_t len)
{
	...
	#ifdef TCSUPPORT_WLAN_8021X_EXT
	snprintf((char *)&buf, sizeof(buf), RADIUS_802_1X_ADDR_FORMAT, MAC2STR(rtapd->own_addr[0]));//send to radius CPE mac address not wireless interface mac.
	#else
	...
	#ifdef TCSUPPORT_WLAN_8021X_EXT //gcyin
	memset(buf, 0, sizeof(buf));
	fp=fopen("/etc/active_pppoe_isp.conf","r");
	if(fp == NULL)
		goto fail;
	while(fgets(tmp, 128, fp) != NULL){//only support one active PPOE PVC
		if(strstr(tmp, "USERNAME") != NULL){
			strcpy(buf,tmp+strlen("USERNAME="));
			break;
		}
	}
	strtok(buf,"\n"); //replace the last char('\n') with NULL.
	if (!Radius_msg_add_attr(msg, RADIUS_ATTR_CALLBACK_NUMBER, buf, strlen((char *)&buf)))
	{
		DBGPRINT(RT_DEBUG_ERROR,"Could not add Callback-number\n");
		fclose(fp);
		goto fail;
	}
	fclose(fp);
...
#ifdef TCSUPPORT_WLAN_8021X_EXT
#ifdef TCSUPPORT_WLAN_8021X_EXT_SHOW_MAC
			sprintf(macaddress, MACSTR, MAC2STR(sta->addr));
			syslog(LOG_INFO,LOG_8021X_SUPPLICANT_RESPONSE,macaddress);			
#else
			syslog(LOG_INFO,LOG_8021X_SUPPLICANT_RESPONSE);
#endif
#endif
...
#ifdef TCSUPPORT_WLAN_8021X_EXT_SHOW_MAC
			sprintf(macaddress, MACSTR, MAC2STR(sta->addr));
			syslog(LOG_INFO,LOG_8021X_SUPPLICANT_START,macaddress);
#else
			syslog(LOG_INFO,LOG_8021X_SUPPLICANT_START);
#endif
#endif
...
#ifdef TCSUPPORT_WLAN_8021X_EXT_SHOW_MAC
			sprintf(macaddress, MACSTR, MAC2STR(sta->addr));
			syslog(LOG_INFO,LOG_8021X_EAPOL_LOGOFF,macaddress);
#else
			syslog(LOG_INFO,LOG_8021X_EAPOL_LOGOFF);
#endif
#endif
...
#ifdef TCSUPPORT_WLAN_8021X_EXT_SHOW_MAC
			sprintf(macaddress, MACSTR, MAC2STR(sta->addr));
			syslog(LOG_INFO,LOG_8021X_RADIUS_ACCEPT,macaddress);
#else
			syslog(LOG_INFO,LOG_8021X_RADIUS_ACCEPT);
#endif
#endif
...
#ifdef TCSUPPORT_WLAN_8021X_EXT_SHOW_MAC
			sprintf(macaddress, MACSTR, MAC2STR(sta->addr));
			syslog(LOG_INFO,LOG_8021X_RADIUS_REJECT,macaddress);
#else
			syslog(LOG_INFO,LOG_8021X_RADIUS_REJECT);
#endif
#endif
...
#ifdef TCSUPPORT_WLAN_8021X_EXT_SHOW_MAC
			sprintf(macaddress, MACSTR, MAC2STR(sta->addr));
			syslog(LOG_INFO,LOG_8021X_RADIUS_CHALLENGE,macaddress);
#else
			syslog(LOG_INFO,LOG_8021X_RADIUS_CHALLENGE);
#endif
#endif
