		...
		case 1:
			...
			if('<% tcWebApi_get("Ddns_Entry", "Active", "s") %>' == '1' && ddns_hostname != ''){
				if(https_port == 443) // if the port number of https is 443, hide it
					$("accessMethod").innerHTML = '<%tcWebApi_Get("String_Entry", "aicloud_disk_case11", "s")%> <a style="font-weight: bolder;text-decoration: underline;word-break:break-all;" href="https://'+ ddns_hostname + ':'+ https_port +' "target="_blank">https://'+ ddns_hostname +'</a><br />';
				else
					$("accessMethod").innerHTML = '<%tcWebApi_Get("String_Entry", "aicloud_disk_case11", "s")%> <a style="font-weight: bolder;text-decoration: underline;word-break:break-all;" href="https://'+ ddns_hostname + ':'+ https_port +' "target="_blank">https://'+ ddns_hostname +':'+ https_port +'</a><br />';

				$("accessMethod").innerHTML += '<%tcWebApi_Get("String_Entry", "aicloud_disk_case12", "s")%>';
			}
			else{
				$("accessMethod").innerHTML = '<%tcWebApi_Get("String_Entry", "aicloud_disk_case21", "s")%><br />';
				$("accessMethod").innerHTML += '<%tcWebApi_Get("String_Entry", "aicloud_disk_case22", "s")%>';
			}
			break;
	}
...
This agreement constitutes the entire agreement between you and ASUS with respect to AiCloud and supersedes all previous communications, representations, understandings and agreements, either oral or written, between you and ASUS regarding AiCloud. This agreement may not be modified or waived except in writing and signed by an officer or other authorized representative of each party. If any provision is held invalid, all other provisions shall remain valid, unless such invalidity would frustrate the purpose of this agreement. The failure of either party to enforce any rights granted hereunder or to take action against the other party in the event of any breach hereunder will not waive that party’s as to subsequent enforcement of rights or subsequent action in the event of future breaches. AiCloud reserves the right to: (a) add or remove functions and features or to provide updates, upgrades or programming fixes to AiCloud with or without prior notice to you; (b) require you to agree to a new agreement in order to use any new version of AiCloud that it releases; or (c) require you to upgrade to a new version of AiCloud by discontinuing service or support to any prior version of AiCloud without notice.
			...
			<div style="background-image:url(/images/Tree/bg_02.png);background-repeat:no-repeat;height:90px;">
				<input class="button_gen" type="button" style="margin-left:27%;margin-top:18px;" onclick="cancel();" value="<%tcWebApi_Get("String_Entry", "CTL_Cancel", "s")%>">
				<input class="button_gen" type="button"  onclick="_confirm();" value="<%tcWebApi_Get("String_Entry", "CTL_ok", "s")%>">	
			</div>
	</div>
							  ...
							  <td bgcolor="#4D595D" valign="top">
									...
									<table width="100%" height="550px" style="border-collapse:collapse;">
									  ...
									  <tr bgcolor="#444f53">
									    <td colspan="5" bgcolor="#444f53" class="cloud_main_radius">
												<div style="padding:10px;width:95%;font-style:italic;font-size:14px;">
													<%tcWebApi_Get("String_Entry", "step_use_aicloud", "s")%>
												  <ol style="-webkit-margin-after: 0em;word-break:normal;">
										        <li style="margin-bottom:7px;">
															<%tcWebApi_Get("String_Entry", "download_aicloud", "s")%><br> 
										        ...
										        <li style="margin-bottom:7px;">															
															<%tcWebApi_Get("String_Entry", "connect_through_wifi", "s")%>
														</li>
										        <li style="margin-bottom:7px;">
															<%tcWebApi_Get("String_Entry", "access_service", "s")%>
														</li>
												  </ol>
										        ...
										        <li style="margin-top:-5px;">
															<span>
																<%tcWebApi_Get("String_Entry", "aicloud_for_privateIP1", "s")%>&nbsp;
																<%tcWebApi_Get("String_Entry", "aicloud_for_privateIP2", "s")%>&nbsp;
	       												<%tcWebApi_Get("String_Entry", "aicloud_for_privateIP3", "s")%>
															</span>
														</li>
												  </ul>
										        ...
										        <li style="margin-top:-5px;">
															<span>
																<%tcWebApi_Get("String_Entry", "aicloud_bandwidth1", "s")%><br>
																<ul>	
																	<li><%tcWebApi_Get("String_Entry", "aicloud_bandwidth2_1", "s")%></li>
																	<li><%tcWebApi_Get("String_Entry", "aicloud_bandwidth2_2", "s")%></li>
																	<li><%tcWebApi_Get("String_Entry", "aicloud_bandwidth2_3", "s")%></li>
																	<li><%tcWebApi_Get("String_Entry", "aicloud_bandwidth2_4", "s")%></li>
																</ul>			
	       												<%tcWebApi_Get("String_Entry", "aicloud_bandwidth3", "s")%>
															</span>
														</li>
												  </ul>												  
									  ...
									  <tr bgcolor="#444f53" width="235px">
									    ...
									    <td>
												<div style="padding:10px;width:95%;font-style:italic;font-size:14px;">
													<%tcWebApi_Get("String_Entry", "aicloud_disk1", "s")%>&nbsp
													<%tcWebApi_Get("String_Entry", "aicloud_disk2", "s")%><br />
									  ...
									  <tr bgcolor="#444f53">
									    ...
									    <td width="">
												<div style="padding:10px;width:95%;font-style:italic;font-size:14px;">
													<%tcWebApi_Get("String_Entry", "smart_access1", "s")%><br />
													<%tcWebApi_Get("String_Entry", "smart_access2", "s")%><br />
													<%tcWebApi_Get("String_Entry", "smart_access3", "s")%>
													<!--br>
													<br>
