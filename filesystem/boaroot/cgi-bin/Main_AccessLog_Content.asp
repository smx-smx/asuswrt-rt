...
<title>ASUS <%tcWebApi_get("String_Entry","Web_Title2","s")%> <% tcWebApi_staticGet("SysInfo_Entry","ProductTitle","s") %> - Access Log</title>
...
if(usb_support != -1){
	...
	var al_bkp_path_fail_flag = '<% tcWebApi_get("Vram_Entry", "al_bkp_path_fail","s") %>';
...
function initial(){
	show_menu();
	if(document.form.al_enable.value == 1){
		document.getElementById("log_field").style.display = "";
		if(usb_support == -1){
			inputCtrl(document.form.al_bkp_enable_x[0], 0);
			inputCtrl(document.form.al_bkp_enable_x[1], 0);
			inputCtrl(document.form.al_bkp_period_x, 0);
			document.getElementById("submitBtn").style.display = "none";
		}
		else{
			usb_disk_options();
			inputCtrl(document.form.al_bkp_enable_x[0], 1);
			inputCtrl(document.form.al_bkp_enable_x[1], 1);
...
function usb_disk_options(){
	free_options(document.form.al_bkp_path_x);
	for(var i=0; i<usb_device_index.length; i++){
		if(usb_device_index[i]=="usb"){
			if(al_bkp_path_orig == usb_mnt_point[i]){
				add_option(document.form.al_bkp_path_x, usb_device_info[i], usb_mnt_point[i], 1);
			}	
			else{
				add_option(document.form.al_bkp_path_x, usb_device_info[i], usb_mnt_point[i], 0);
			}
		}
	}
	
	if(al_bkp_path_fail_flag == 1){	//show al_bkp_path not exist hint
...
function redirect(){
	document.location.href = "/Main_AccessLog_Content.asp";
}

function back_syslog(){
	var cfg = '/access_log.txt';
	var code = 'location.assign(\"' + cfg + '\")';
	eval(code);
}

function change_al_bkp_enable(flag){
	if(flag == 1){
...
function updateUSBStatus(){	
	if((rc_support.search("usbX") == -1 || rc_support.search("usbX1") >= 0) && usb_path1.search("storage") == -1){	//single USB port		
		...
		document.form.al_bkp_enable_x[0].disabled = true;
		document.form.al_bkp_enable_x[1].checked = true;
	}
		...
		document.form.al_bkp_enable_x[0].disabled = false;
	}					
}

function applyRule(){
	if(document.form.al_clear_x.checked == true)
...
<form method="post" name="form" action="/Main_AccessLog_Content.asp" target="hidden_frame">
						...
						<table width="760px" border="0" cellpadding="5" cellspacing="0" bordercolor="#6b8fa3"  class="FormTitle" id="FormTitle">		
							<tr>
								<td bgcolor="#4D595D" colspan="3" valign="top">
									<div>&nbsp;</div>
									<div id="content_title" class="formfonttitle"><% tcWebApi_Get("String_Entry", "System_Log", "s") %> - Access Log</div>	<!-- untranslated -->
									<div style="margin-left:5px;margin-top:10px;margin-bottom:10px"><img src="/images/New_ui/export/line_export.png"></div>
									<div class="formfontdesc">
										Access Log shows clients connection access.<!-- untranslated -->
									</div>
									<div style="margin:5px">
										<table style="margin-left:0px;" width="95%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3" class="FormTable">
											<tr>
												<th><!--a class="hintstyle" href="javascript:void(0);" onClick="openHint(20,6);"-->Enable Access Log<!--/a--></th>
												<td>
												<div align="center" class="left" style="width:94px; float:left; cursor:pointer;" id="al_enable_x"></div>
													<script type="text/javascript">
														$j('#al_enable_x').iphoneSwitch('<%tcWebApi_get("AccessLog_Entry","al_enable","s")%>',
															function(){
																document.form.logFlag.value = 1;
																document.form.al_enable.value = 1;
																showLoading(3);
																setTimeout("redirect();", 3000);
																document.form.submit();
															},
															function(){
																document.form.logFlag.value = 1;
																document.form.al_enable.value = 0;
																showLoading(3);
																setTimeout("redirect();", 3000);
																document.form.submit();
															}
											...
											<tr>
												...
												<td>
													<input type="radio" name="al_bkp_enable_x" class="input" value="1" onclick="change_al_bkp_enable(1);" <% if tcWebApi_get("AccessLog_Entry","al_bkp_enable","h") = "1" then asp_Write("checked") end if %>><%tcWebApi_get("String_Entry","checkbox_Yes","s")%>
													<input type="radio" name="al_bkp_enable_x" class="input" value="0" onclick="change_al_bkp_enable(0);" <% if tcWebApi_get("AccessLog_Entry","al_bkp_enable","h") = "0" then asp_Write("checked") end if %>><%tcWebApi_get("String_Entry","checkbox_No","s")%>
													<span id="be_lack_storage" style="display:none;color:#FC0">* No USB disk plug-in.</span>													
													<br>
													<div id="al_bkp_path_div">
														USB Disk :&nbsp;<select name="al_bkp_path_x" class="input_option" onchange="clear_bkup_path_fail();"></select>														
											...
											<tr>
												...
												<td>
													<select name="al_bkp_period_x" class="input_option">
														<option value="3600" <%if tcWebApi_get("AccessLog_Entry","al_bkp_period","h") = "3600" then asp_Write("selected") end if%> >1</option>
														<option value="21600" <%if tcWebApi_get("AccessLog_Entry","al_bkp_period","h") = "21600" then asp_Write("selected") end if%> >6</option>
														<option value="43200" <%if tcWebApi_get("AccessLog_Entry","al_bkp_period","h") = "43200" then asp_Write("selected") end if%> >12</option>
														<option value="86400" <%if tcWebApi_get("AccessLog_Entry","al_bkp_period","h") = "86400" then asp_Write("selected") end if%> >24</option>
											...
											<tr bgcolor="#4D595D">
												<td valign="top">
													...
													<div style="margin-top:8px">
														<textarea style="width:99%; font-family:'Courier New', Courier, mono; font-size:12px;background:#475A5F;color:#FFFFFF;" cols="63" rows="25" wrap="off" readonly><% nvram_dump("access_log.log","") %></textarea>
													</div>													
												</td>
											</tr>
											</table>
