...
function initial(){
	...
	if(document.form.wl_unit[0].selected == true)
	{
		...
		for(i=0; i<algos.length; i++){
			switch(i) {
				case 0:
					document.form.wl_nmode_x[i] = new Option(algos[i], "8");
					break;
				case 1:
...
<option class="content_input_fd" value="1" selected>Enable</option>
	...
	<tr>
		...
		<td>
			<select name="wl_nmode_x" class="input_option" onChange="wireless_mode_change(this);"> <!-- limit_auth_method(this);return change_common(this, 'WLANConfig11b', 'wl_nmode_x'); -->
	...
	<tr id="wl_channel_field">
		...
		<td>
			<select id="wl_channel" name="wl_channel" class="input_option" onChange="change_channel();"></select>
