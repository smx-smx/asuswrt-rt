...
_find_act_devs(){
	...
	for dev in $tty_devs; do
		path=`/userfs/bin/tcapi get USB_Entry usb_path_$dev`
...
/userfs/bin/tcapi set USBModem_Entry usb_modem_act_int "$first_int_dev" &
/userfs/bin/tcapi set USBModem_Entry usb_modem_act_bulk "$first_bulk_dev" &
