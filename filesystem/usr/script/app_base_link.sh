...
if [ "$APPS_MOUNTED_TYPE" != "vfat" ]; then
	...
	user_account=`tcapi get Account_Entry0 username`
...
# ipkg-opt
ln -sf $APP_BIN/ipkg $APP_LINK_BIN/ipkg
...
if [ "$pkg_type" = "arm" ]; then
	...
	ln -sf $APP_LIB/libstdc++.so.6.0.10 $APP_LINK_LIB/libstdc++.so.6
	ln -sf $APP_LIB/libstdc++.so.6.0.10 $APP_LINK_LIB/libstdc++.so
