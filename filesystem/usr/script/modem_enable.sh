...
modem_mode=`/userfs/bin/tcapi get USBModem_Entry modem_mode`
modem_roaming=`/userfs/bin/tcapi get USBModem_Entry modem_roaming`
modem_roaming_mode=`/userfs/bin/tcapi get USBModem_Entry modem_roaming_mode`
modem_roaming_isp=`/userfs/bin/tcapi get USBModem_Entry modem_roaming_isp`
modem_roaming_imsi=`/userfs/bin/tcapi get USBModem_Entry modem_roaming_imsi`
modem_autoapn=`/userfs/bin/tcapi get USBModem_Entry modem_autoapn`
modem_auto_spn=`/userfs/bin/tcapi get USBModem_Entry usb_modem_auto_spn`
modem_act_path=`/userfs/bin/tcapi get USBModem_Entry usb_modem_act_path`
...
modem_imsi=`/userfs/bin/tcapi get USBModem_Entry usb_modem_act_imsi`
modem_pin=`/userfs/bin/tcapi get USBModem_Entry modem_pincode`
modem_pdp=`/userfs/bin/tcapi get USBModem_Entry modem_pdp`
modem_isp=`/userfs/bin/tcapi get USBModem_Entry modem_isp`
modem_spn=`/userfs/bin/tcapi get USBModem_Entry modem_spn`
modem_apn=`/userfs/bin/tcapi get USBModem_Entry modem_apn`
modem_authmode=`/userfs/bin/tcapi get USBModem_Entry modem_authmode`
modem_user=`/userfs/bin/tcapi get USBModem_Entry modem_user`
modem_pass=`/userfs/bin/tcapi get USBModem_Entry modem_pass`
modem_apn_v6=`/userfs/bin/tcapi get USBModem_Entry modem_apn_v6`
modem_authmode_v6=`/userfs/bin/tcapi get USBModem_Entry modem_authmode_v6`
modem_user_v6=`/userfs/bin/tcapi get USBModem_Entry modem_user_v6`
modem_pass_v6=`/userfs/bin/tcapi get USBModem_Entry modem_pass_v6`
...
_find_usb3_path(){
	all_paths=`/userfs/bin/tcapi get USB_Entry xhci_ports`
...
_find_usb2_path(){
	all_paths=`/userfs/bin/tcapi get USB_Entry ehci_ports`
...
_find_usb1_path(){
	all_paths=`/userfs/bin/tcapi get USB_Entry ohci_ports`
...
if [ "$modem_type" == "tty" -o "$modem_type" == "qmi" -o "$modem_type" == "mbim" -o "$modem_type" == "gobi" ] || [ "$usb_gobi2" == "1" ]; then
	nvram_reset=`/userfs/bin/tcapi get USBModem_Entry modem_act_reset`
	#if [ "$nvram_reset" == "1" -o "$modem_vid" == "6610" -a "$modem_pid" == "644" ]; then # ZTE MF880
	if [ "$nvram_reset" == "1" ]; then
		# Reset modem.
		echo "Reset modem."
		/userfs/bin/tcapi set USBModem_Entry usb_modem_act_reset 1 &
		/userfs/bin/tcapi set USBModem_Entry usb_modem_act_reset_path "$modem_act_path" &
		...
		if [ "$at_ret" == "OK" ]; then
			...
			if [ "$modem_act_node" != "" ]; then
				...
				/userfs/bin/tcapi set USBModem_Entry usb_modem_act_reset 0 &
				/userfs/bin/tcapi set USBModem_Entry usb_modem_act_reset_path "" &
				exit 5
			fi

			echo "Reset modem: wait the modem to wake up..."
			reset_flag=`/userfs/bin/tcapi get USBModem_Entry usb_modem_act_reset`
			tries=1
	...
	ret=`/userfs/bin/tcapi get USBModem_Entry usb_modem_act_sim`
	if [ "$ret" != "1" ]; then
	...
	if [ "$modem_vid" == "8193" ];then
		...
		if [ "$modem_dev" == "" ]; then
			...
			modem_dev=`/userfs/bin/tcapi get USB_Entry usb_path"$path"_act`
			if [ "$modem_dev" == "" ]; then
				echo "Couldn't get the QMI dev..."
				exit 8
			fi

			/userfs/bin/tcapi set USBModem_Entry usb_modem_act_dev $modem_dev &
		...
		if [ "$ret" != "0" ]; then
			...
			/userfs/bin/tcapi set USBModem_Entry usb_modem_reset_huawei 1 &
			...
			/userfs/bin/tcapi set USBModem_Entry usb_modem_reset_huawei "" &
		fi
		...
		if [ "$at_cgnws" != "" ]; then
			...
			if [ "$mcc$mnc" == "$target" ]; then
				...
				if [ "$modem_spn" == "" -a "$spn" != "" -a "$spn" != "NULL" ]; then
					/userfs/bin/tcapi set USBModem_Entry modem_spn $spn &
				fi

				# useless temparily.
				#isp=`echo -n "$at_cgnws" |awk '{FS=","; print $8}' 2>/dev/null` # ISP long name
				#if [ "$isp" != "" -a "$isp" != "NULL" ]; then
				#	/userfs/bin/tcapi set USBModem_Entry modem_isp $isp &
				#else
