...
tcapi set Apps_Entry apps_state_update 1 # UPDATING
SQ_TEST=`tcapi get Apps_Entry apps_sq`
i=0
while [ $i -lt $row_num ]; do
	...
	#	wget --spider $wget_options $server_name/$list			#removed, not support --spider in current busybox version.
	#	if [ "$?" = "0" ]; then
...
tcapi set Apps_Entry apps_state_update 2 # FINISHED
