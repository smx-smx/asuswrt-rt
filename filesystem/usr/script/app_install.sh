...
_download_package(){
	if [ "$1" = "uclibc-opt" ] || [ "$1" = "ipkg-opt" ]; then
		...
		if [ "$1" = "downloadmaster" ] && [ -z "$apps_from_internet" ]; then
			...
			installed_ipk_path=`ls $apps_local_space/mediaserver*`
		elif [ "$1" = "asuslighttpd" ] && [ -z "$apps_from_internet" ]; then
			installed_ipk_path=`ls $apps_local_space/asuslighttpd*`
		elif [ "$1" = "asusffmpeg" ] && [ -z "$apps_from_internet" ]; then
			installed_ipk_path=`ls $apps_local_space/asusffmpeg*`
		fi
		...
		if [ "$need_download" = "1" ]; then
			...
			if [ -z "$pkg_server" ]; then
				tcapi set Apps_Entry apps_state_errmsg "No ipkg server address!!"
			...
			tcapi set Apps_Entry apps_download_file "$ipk_file_name"
			...
			if [ -z "$wget_pid" ] || [ $wget_pid -lt 1 ]; then
				rm -rf $target
				sync
				tcapi set Apps_Entry apps_state_errmsg "Cannot find running wget!!"
				tcapi set Apps_Entry apps_state_error 6
				return 1
			fi
			i=0
			...
			if [ -z "$percent" ] || [ $percent -ne 100 ]; then
				rm -rf $target
				sync
				tcapi set Apps_Entry apps_state_errmsg "No $target size found!!"
				tcapi set Apps_Entry apps_state_error 6
				return 1
			fi
...
tcapi set Apps_Entry apps_state_install 0 # INITIALIZING
...
SWAP_THRESHOLD=`tcapi get Apps_Entry apps_swap_threshold`
SWAP_FILE=`tcapi get Apps_Entry apps_swap_file`
SWAP_SIZE=`tcapi get Apps_Entry apps_swap_size`
...
if [ -z "$APPS_DEV" ]; then
	...
	tcapi set Apps_Entry apps_dev "$APPS_DEV"
	tcapi set Apps_Entry apps_mounted_path "$APPS_MOUNTED_PATH"
fi
APPS_INSTALL_PATH=$APPS_MOUNTED_PATH/$APPS_INSTALL_FOLDER


tcapi set Apps_Entry apps_state_install 1 # CHECKING_PARTITION
...
if [ "$?" != "0" ]; then
	...
	tcapi set Apps_Entry apps_state_install 5 # FINISHED
	exit 0
fi

tcapi set Apps_Entry apps_state_install 2 # CHECKING_SWAP
if [ "$SWAP_ENABLE" != "1" ]; then
	...
	if [ "$SWAP_THRESHOLD" = "" ] || [ $mem_size -lt $SWAP_THRESHOLD ]; then
		...
		if [ $pool_size -gt $SWAP_SIZE ]; then
			...
			tcapi set Apps_Entry apps_state_error 3
			exit 1
		fi
	fi
fi

tcapi set Apps_Entry apps_state_install 3 # DOWNLOADING
...
tcapi set Apps_Entry apps_state_install 4 # INSTALLING
if [ "$1" = "downloadmaster" ] && [ -z "$apps_from_internet" ]; then
...
for file in $target_file; do
	...
	if [ "$result" = "error" ]; then
		...
		tcapi set Apps_Entry apps_state_errmsg "app_install:Fail to install the package: $file!"
		exit 1
	else
		rm -rf $file
		rm -f $install_log
...
rc_call nasapps 2 #rc rc_service restart_nasapps
