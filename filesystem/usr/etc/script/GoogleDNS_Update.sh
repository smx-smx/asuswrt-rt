...
Successful()
{
	...
	tcapi set Vram_Entry ddns_return_code 200
	tcapi set Vram_Entry ddns_return_code_chk 200
	ddns_updated
}
...
Success_NoChange()
{
	...
	tcapi set Vram_Entry ddns_return_code_chk no_change
	ddns_updated
}
...
badauth()
{
	...
	tcapi set Vram_Entry ddns_return_code auth_fail
	tcapi set Vram_Entry ddns_return_code_chk auth_fail
}

unvalid_request()
{
	#Code:401, LANHostConfig_x_DDNS_alarm_10=Unauthorized registration request! 
	tcapi set Vram_Entry ddns_return_code 401
	tcapi set Vram_Entry ddns_return_code_chk 401
}

otherfail()
{
	#Code:unknow_error, LANHostConfig_x_DDNS_alarm_2=Request error! Please try again.
	tcapi set Vram_Entry ddns_return_code unknown_error
	tcapi set Vram_Entry ddns_return_code_chk unknown_error
}
...
main()
{
	...
	echo -n "`date +%s`,$CurrWANIP" > /tmp/ddns.cache

	wget --no-check-certificate "https://$USER:$PASSWD@domains.google.com/nic/update?hostname=$HOST&myip=$CurrWANIP" -O $RESULT_PATH
